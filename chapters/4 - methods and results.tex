...
As shown in eq.~\ref{eqn:theta average} the gravimetric measurement is conducted by measuring the equilibrium angle of the torsional pendulum. The measurement is limited both by the detector resolution and the quantum uncertainty due to thermal energy (eq.~\ref{eqn:Brownian uncertainty 3}). Assuming a 12-bit detector range, and room temperature the measurement is given by:
\begin{equation}
\frac{M}{r_0^3} = \frac{\overline{\theta}\pm \delta\theta}{\frac{3GT^2cos\theta sin\theta}{4\pi^2 }} \approx \frac{\overline{\theta}\pm \delta\theta}{2\cdot 10^{-8}}
\label{eqn:theta average}
\end{equation} 

\begin{equation}
\delta\theta = \sqrt{\frac{k_B T}{\kappa}}+ \frac{2\cdot \theta_{max}}{12-bit}\approx \approx 4\cdot 10^{-8} [rad] 
\label{eqn:measurement uncertainty}
\end{equation}
\end{document}