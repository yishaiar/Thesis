...
\end{equation}
Where $\Theta_{max}$ is the is the the light sources' maximal flux with a given efficiency $\eta$. As shown in eq.~\ref{eqn:damping_torque_pid}, to successfully damp the torsional pendulum the maximal flux in each light source $\Theta_{max}$ is given by:
\begin{equation}
