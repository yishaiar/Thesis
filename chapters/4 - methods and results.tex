...
As shown previously, there are environment noises affecting the torsional pendulum (eq.~\ref{eqn:Brownian power}, eq.~\ref{eqn:heat conduction}, eq.~\ref{eqn:acoustic power}, eq.~\ref{eqn: Stefan–Boltzmann power}). Assuming gas and environment temperature of $T = 298K$ and pendulum temperature $T_0 = 0K$ (maximal noise power), the noise power is very large. 
\par\noindent
Since the thermal and Brownian energy arrives by particles with random direction and phase the power can be modelled as a Poisson process with the RMS of the number of gas particles $\sqrt{N}$, resulting with an effective pressure dependency of the power $P_{RMS}$:
\begin{equation}
P_{RMS} =\sqrt{N}\frac{k_B T}{V}=\sqrt{P}\cdot \sqrt\frac{k_B T}{V} \approx \sqrt{P}\cdot 3\cdot10^{-9}      \label{eqn:P_RMS}
\end{equation}
...
\begin{easylist}
& Black body radiation \cite{WOODS201444};
&& $p=0.121[W]$.
& Acoustic waves;
&& $p=1.14\cdot 10^{-28}[W]$.
&& $p_{RMS}=1.14\cdot 10^{-28}[W]$.
\end{easylist}
The measurement system is placed inside a acoustic box which is also a Faraday cage with 76 mm thickness, that can block magnetic fields with frequencies $f \ge 30 [Hz]$ from the environment. Thus, the acoustic box both reduces acoustic waves and magnetic noise from the environment.
\par\noindent
The vacuum chamber is a second cage that further blocks magnetic noise from the electronic system placed inside the acoustic box. The vacuum chambers is made out of approximately 3 mm thick stainless steel, blocking magnetic fields with frequencies $f\ge 20 [KHz]$ and reducing magnetic noise with lower frequencies.
\par\noindent


Although the noise power can be very large, since it arrives through particles with random directions it can be modelled as a Poisson process with the RMS of the number of particles $\sqrt{N}$, resulting with an effective pressure dependency of the power $P_{RMS}$ which is given by:
\begin{equation}
P_{RMS} =\sqrt{N}\frac{k_B T}{V}=\sqrt{P}\cdot \sqrt\frac{k_B T}{V} = 2.94\cdot10^{-9}\cdot\sqrt{P}      \label{eqn:P_RMS}
\end{equation}
