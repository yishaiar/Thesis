...
As shown in eq.~\ref{eqn:Brownian uncertainty 3}, without technical noises the fundamental quantum uncertainty due to thermal energy at room temperature results with amplitude of  $\theta_{max}  \approx 4\cdot 10^{-8} [rad] $.
...
\begin{figure}[htbp]
	...
	\label{fig:measured oscillation angle1}
\end{figure}
\begin{figure}[htbp]
	\centering
	\fbox{\includegraphics[scale=0.17]{\main/images/4 - methods and results/duty_cycle.png}}
	\caption[The measured oscillation angle]{The measured oscillation angle}
	\label{fig:measured oscillation angle2}
\end{figure}

Shown in fig.~\ref{fig:measured oscillation angle1} are the damped torsional pendulum oscillations, using the PID. Since the damping is depended on the amplitude size (eq.~\ref{eqn:pid damping coefficient}), the damping accelerates as the amplitude gets smaller. Due to tourqe power and phase delay, the PID has a lower limit to the damping angle. 
\par\noindent
Shown in fig.~\ref{fig:measured oscillation angle2}, are the damped oscillation after the PID reached its limit. The PID is able to keep the small amplitude constant over time, enabling to integrate over the results, and have  gravitational measurements.

The effective temperature of the torsional pendulum is given by (eq.~\ref{eqn:Brownian uncertainty 3}): 
\begin{equation}
T(t) = \theta_{max}(t)^2\frac{\kappa}{k_B} \label{eqn:temperature}
\end{equation}
