...
\end{equation}
The PID continuously calculates the oscillator's error value from a defined set point, damping the error to zero. The PID mainly acts as a frictional torque, gradually working when the oscillations are at the maximum velocity slowing them down, given by:
\begin{equation}
...
\begin{equation}
\gamma  =  \frac{max(\tau_{PID})}{\dot{\theta}_{max}} =  \frac{max(\tau_{PID})}{\theta_{max}\cdot\frac{2\pi}{T}} =         \label{eqn:damped_pid_motion_equation_2}
\end{equation}
