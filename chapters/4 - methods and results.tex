...
\label{eqn:Q}
\end{equation}
It can be seen from eq.~\ref{eqn:Q} that $Q$ is highly depended on the amplitude size $\theta_{max}$. Even when assuming no environmental noises $p=0$, the amplitudes at which the oscillator is not highly underdamped $Q < 1$ (the damping would effect the oscillations) are given by:  
\begin{equation}
