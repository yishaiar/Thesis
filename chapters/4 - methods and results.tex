...
It can be seen from eq.~\ref{eqn:Q} that when $Q<0$ the coupled noise power is larger than the work done by the PID, causing the oscillator to be driven. From the actual measured damped amplitudes of $\theta_{max}= 2\cdot 10^{-7} - 5\cdot 10^{-8} [rad]$ one can estimate the maximal effective noise coupled to the torsional pendulum.
