...
\begin{equation}
\gamma  =  \frac{max(\tau_{PID})}{\dot{\theta}_{max}} =  \frac{max(\tau_{PID})}{\theta_{max}\cdot\frac{2\pi}{T}} =\frac{1}{\theta_{max}}\cdot \frac{max(\tau_{PID})\cdot T}{2\pi}          \label{eqn:damped_pid_motion_equation_2}
\end{equation}
As shown in eq.~\ref{eqn:damping_time} the damping time $\tau$ and damping ratio $\xi$ are given by:
\begin{equation}
\tau = \frac{1}{\xi\omega_0} =  \frac{2I}{\gamma} = \theta_{max} \cdot \frac{4 \pi I}{max(\tau_{PID})\cdot T} = \theta_{max} \cdot \frac{4 \pi (2ml^2)}{max(\tau_{PID})\cdot T} \label{eqn:damping_time_pid}
\end{equation}
\par\noindent
