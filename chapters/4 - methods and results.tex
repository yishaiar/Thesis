...
\par\noindent 
Also, since the Arduino clock is connected to all PWM pins, they are all in sync. When all PWM pins have the same duty cycle, they all have the same voltage, frequency and phase. Even though the pins varies in time, they are constant voltage supplies compare to each other, allowing to connect all in parallel and increase the output current $I = \sum I_i$. 
...
\end{equation}
Since the forward voltage is logarithm of the current, it varies very slow and is approximately constant over wide current range. Having a constant forward voltage results with large changes in the LED current due to small changes in the driving circuit supply voltage $V$. LED driving circuit consisted of resistor $R$ in series with the LED stabilizes the current, the circuit current is given by:
\begin{equation}
I =\frac{V-V_d}{R} \label{eqn:led circuit}
\end{equation}
The LED forward voltage direction is inverse to the current. Since the supply voltage $V$ is composed of $N$ voltage supplies in parallel the LED output power is given by:
\begin{equation}
P = I_{total}\cdot (-V_d) = (NI)\cdot (-V_d) = N(-V_d)\frac{V-V_d}{R}  =N(-V_d)\frac{\frac{V_d}{100}\cdot D(t)-V_d}{R} \label{eqn:led power}
\end{equation}
