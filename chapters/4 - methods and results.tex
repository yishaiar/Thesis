...
\end{equation}
\subsubsection{Arduino driving circuit}
...
\end{equation}
The driving circuit is composed of supply voltage with 6 PWM Arduino pins in parallel, and a resistor $R = 200\Omega$, for a blue LED $V_l\approx 4.5V$ and the LED output power is given by:
\begin{equation}
P(t) = I_{total}\cdot V_l = NI\cdot V_l = N V_l \frac{V-V_l}{R}\cdot  =N V_l\frac{\frac{5}{100}\cdot D(t)-V_l}{R} \approx N V_l\frac{\frac{5}{100}\cdot D(t)}{R} = 0.007\cdot D(t) \label{eqn:led power}
\end{equation}
Where the Arduino response varies 
