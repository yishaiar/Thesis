...
A PID feed-back loop is damping the torsional pendulum by continuously calculating the error value of the measured signal from a defined set point $e(t) =SP -\theta(t) $, and damp it to zero. The control system aims to reach $SP$ by critically damping of the process (the torsional pendulum). A well tuned PID reaches the $SP$ fast and accurate, while resisting forces trying to move the process from the $SP$. 
\par\noindent
In control theory there are two wanted conflicting properties; an accurate response (small overshoot), and small risetime (fast response). Overshoot is when the output signal (the actual PID response) exceeds the target value (the wanted response), thus the response is not accurate.  Overshoot is usually measured in percentage overshoot $PO$, for second order systems $PO$ is given by:
\begin{equation}
...
\end{equation}
\par\noindent
Where $\gamma$ is the PID damping coefficient, since $SP$ is constant, $\dot{e}(t)$ is the the torsional pendulum's oscillations velosity $\dot{\theta}(t)$. The torsional pendulum is a simple harmonic oscillator with maximal velocity $\dot{\theta}_{max} =\frac{2\pi}{T} \theta_{max}$ (see eq.~\ref{eqn:undamped_motion_equation_solved}) and initial oscillations angle $\theta_{max}$. The PID damping coefficient $\gamma$ is a constant value given by:
\begin{equation}
...
\end{equation}
Where $max(\tau_{PID})$ is the maximal torque exerted by the PID, as shown in eq.~\ref{eqn:damping_time} the damping time $\tau$ and damping ratio $\xi$ are given by:
\begin{equation}
\tau = \frac{T}{2 \pi \xi } =  \frac{2I}{\gamma} = \frac{\theta_{max}}{ max(\tau_{PID})} \cdot \frac{4\pi I}{T}  \label{eqn:damping_time_pid}
\end{equation}
The PID damping $\gamma$, damping time $\tau$ and damping type $\xi$ depend on the ratio $\frac{\theta_{max}}{max(\tau_{PID})}$. If $\theta_{max}$ is too large compare to the external torque, the PID affect is negligible, causing an extremely underdamped pendulum with an infinite damping time. The damping ratio $\xi$ when the pendulum is critically damped by $\tau_{critical}$ is given by (eq.~\ref{eqn:critically_damped_motion_equation}):
\begin{equation}
\xi = \frac{T^2}{ 8 \pi^2 I }\cdot\frac{ \tau_{critical}}{\theta_{max}} = 1 
\label{eqn:damping_ratio_pid}
\end{equation}
In order to critically damp the torsional pendulum by the PID, the PID torque $\tau_{PID}$ needs to be able to exert torques as large as the critical damping torque $max(\tau_{PID}) \geq  \tau_{critical}$, given by:
\begin{equation}
...
\end{equation}
\par\noindent
The PID controlled torque $\tau_{PID}(t)$ needs a maximal torque given by eq.~\ref{eqn:damping_torque_pid}. Since, it is proportional to the oscillations velocity (eq.~\ref{eqn:friction_torque_pid}), it needs a high speed modulation compare to the oscillations period $T$ (to prevent phase delay), and high dynamic range compare to oscillations velosity $\dot{\theta}_{max}$ (to avoid overshoot). The sufficient modulation speed and range are unknown.
...
\end{equation}
The radiation-pressure net torque is modulated by changing the difference between the light sources' flux, according to eq.~\ref{eqn:damping_torque_pid} the maximal net torque $\tau_{max}$ is given by: 
\begin{equation}
...
The modulated light sources are composed of two LED light sources driven by an Arduino microcontroller, resulting with a linear response of the torque to the PID control (see eq.~\ref{eqn:radiation torque}) and minimizing overshoot, while minimizing phase delay by having a fast response time.
\par\noindent
...
The light emitting diode (LED) is a semiconductor light source with high power, and a long lifetime. A forward voltage applied to a p-n junction, causes electron injection that recombinate with holes. The recombination releases energy in form of spontaneous emission photons (incoherent light). Due to the electrons life time, the LED could be modulated up to $100MHz$.
\par\noindent
...
\end{equation}
\par\noindent
In order to overcome the led incoherent profile and large illumination angle, the light is coupled to a light guide, which is a pipe made of thin filaments causing internal reflections designe to illuminate small areas, regardless of the spectral characteristics of the light source. The light guide efficiency is mainly dependent on the cross section and length of the lightguide, making it ideal to overcome focusing problems. 
\par\noindent
Due to coupling efficiency and size difference between the output beam from the lightguide and the pendulum sides, the efficiency of flux hitting the pendulum was estimated to be $\eta = 0.7$, thus the PID torque $\tau_{PID}(t)$ is given by:
\begin{equation}
\tau_{PID}(t) \approx \frac{2l\eta}{{c}} (\Theta_1(t) -\Theta_2(t)) \approx \frac{2l\eta}{{c}} \cdot\frac{N V_l V_d}{100R}(D_1(t) -D_2(t))  \approx   3.56^{-12}(D_1(t) -D_2(t)) 
\label{eqn:led torque}
\end{equation}
\par\noindent
Where the torque is calculated using eq.~\ref{eqn:led power}, with controlled duty cycle $D(t)$ varying between $0-100$ with 8bit resolution, thus generating torques up to $\tau = 7\cdot10^-{10} [N\cdot m]$ with modulation steps of $\Delta\tau = 1.4\cdot10^-{12} [N\cdot m]$. As shown in eq.~\ref{eqn:pwm frequency}), the LED could be modulated up to $500[Hz]$ resulting with fast modulated external force for the PID algorithm. Also, since the led power could be approximated using linear approximation, it allows to control the approximated power output by changing the PID gain values.  
