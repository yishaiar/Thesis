...
\par\noindent
Since the thermal and Brownian energy arrives by particles with random direction and phase the power can be modelled as a Poisson process with the RMS of the number of gas particles $\sqrt{N}$, resulting with a significantly lower effective power $P_{RMS}$. 
\par\noindent
The average energy of a black body radiation photon is given by $E=2.7 k_B T$, when the pendulum temperature is  $T_0 = 0K$, the black body radiation effective power is given by \cite{WOODS201444}:
\begin{equation}
p_{RMS} = \sqrt{N}\cdot E = \sqrt{\frac{p}{E}}\cdot E = \sqrt{\frac{\sigma\epsilon(T_0=0) A T^4}{2.7 k_B T}}\cdot 2.7 k_B T   \label{eqn: max Stefan–Boltzmann power}
\end{equation}
 although we don't have an accurate estimation of the particles RMS, we estimate a similar reduction of the effective power. 
...
\end{easylist}
\end{multicols}
The measurement system is placed inside an acoustic box, further reducing the acoustic power.    
\par\noindent
