...
\begin{equation}
W_{PID} = \int_0^T\tau_{PID}(t)\cdot\dot{\theta}(t)dt = \int_0^T\gamma\dot{\theta}\cdot\dot{\theta}(t)dt = \int_0^T\frac{max(\tau_{PID})}{\theta_{max}} \frac{ T}{2\pi}(\frac{2\pi}{T}\theta_{max})^2 sin(\frac{2\pi}{T}t)dt  
\label{eqn:pid work}
\end{equation}
\begin{equation}
...
\end{equation}
\begin{equation}
Q = \frac{0.5\kappa \theta_{max}^2}{\theta_{max}\cdot\pi\tau_{max}} \approx frac{1.3\cdot 10^{-6} \theta_{max}}{1.0\cdot 10^{-11}}\approx frac{1.3\cdot 10^{5} \theta_{max}}{1.0\cdot 10^{-11}}
\label{eqn:pid work}
\end{equation}
\end{document}