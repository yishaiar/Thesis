...
\end{equation}
The effective noise power dependent on the gas pressure and the environment temperature (room temperature $T$). The effective noise power is given by:
\begin{equation}
p_{eff}  = \frac{12AP}{m_1}\sqrt{\frac{3 k_B T M}{ N_A }} + \frac{2.7A k_B}{ c^2 m_1} (4-3\epsilon)\sigma T^5 \approx 4.4\cdot 10^{-23}P +1.4\cdot 10^{-47}T^5 \approx 4.4\cdot 10^{-25}  [W]
\label{eqn:net power}
\end{equation} 
