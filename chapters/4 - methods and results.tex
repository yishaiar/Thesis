...
\end{equation}
In order to critically damp the torsional pendulum by the PID, the PID torque $\tau_{PID}$ needs to be able to exert torques as large as the critical damping torque $\tau_{PID}\geq  \tau_{critical}$. The PID torque $\tau_{PID}$ is given by:
\begin{equation}
