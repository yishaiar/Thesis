...
\par\noindent
The torsional pendulum design, shown in fig.~\ref{fig:pendulum front}, is designed to have an optimal mass sensitivity to field torque, given by eq.~\ref{eqn:gravitation_torque_equilibrium}:
\begin{equation}
m \approx \overline{\theta}  \cdot \frac{4\pi^2 r_0^3}{3GT^2cos\theta sin\theta} \approx  \frac{\kappa}{\tau_g}  \cdot \frac{4\pi^2 r_0^3}{3GT^2cos\theta sin\theta}
  \label{eqn:mass measure}
\end{equation} 

 to balance the achievement of large angle sensitivity to field torque (large string torsion coefficient $\kappa$, ) while maintaining high mass sensitivity to angle (longer oscillation time period $T$, eq.~\ref{eqn:theta average_2}). 
\par\noindent
