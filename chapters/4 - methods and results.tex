...
\end{equation}
\begin{equation}
W_t=T = \frac{\tau_{max}\theta_{max}2\pi}{T} \frac{T}{2}-p_{noise}\cdot T = \theta_{max}\cdot\pi\tau_{max} -p_{noise}\cdot T
\label{eqn:pid work}
\end{equation}
\begin{equation}
Q =2\pi\cdot\frac{energy\quad stored}{energy\quad dissipated\quad per\quad cycle}=2\pi\cdot \frac{U}{W_T}= 2\pi\cdot\frac{0.5\kappa \theta_{max}^2}{\theta_{max}\cdot\pi\tau_{max}} \approx \frac{1.3\cdot 10^{-6} \theta_{max}}{1.0\cdot 10^{-11}}\approx 1.3\cdot 10^{5}\cdot \theta_{max}
\label{eqn:pid work}
\end{equation}
\end{document}