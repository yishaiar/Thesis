...
\end{equation}
Where $\gamma$ is the PID damping coefficient, as shown in eq.~\ref{eqn:damped_motion_equation}) the system is a damped oscillator, with an external force correction, which inserts torque to the oscillator, given by:
\begin{equation}
\kappa\theta - \gamma\dot{\theta}  + I\ddot{\theta} = 0   \label{eqn:damped_pid_motion_equation}
\end{equation}
\begin{equation}
