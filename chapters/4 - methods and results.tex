...
\par\noindent
Due to tourqe power, phase delay due to other modes of the torsional pendulum, and not having the right tuning parameters (which are achieved experimentally), the PID has a lower limit to the damping angle. The PID is able to damp up to $\theta_{max}\approx 5\cdot 10^{-8}[rad]$.
\par\noindent
The PID is able to keep a small amplitude of $\overline{\theta_{max}}\approx 1\cdot 10^{-7}[rad]$ constant over time, enabling to integrate over the results, and have gravitational measurements of..
\par\noindent
