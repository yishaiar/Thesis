...
\begin{equation}
Q =2\pi\cdot\frac{maximum\ energy\ stored}{energy\ dissipated\ per\ cycle}=2\pi\cdot \frac{U}{W_T}= 2\pi\cdot\frac{\frac{\kappa\theta_{max}^2}{2}}{\frac{\theta_{max}\cdot\pi\tau_{max}\cdot T -p_{noise}}{T}} =
\pi T\cdot\frac{\kappa\theta_{max}^2}{\theta_{max}\cdot\pi\tau_{max}\cdot T -p_{noise}}
\label{eqn:pid work}
\end{equation} 
\begin{equation}
