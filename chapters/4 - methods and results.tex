...
\end{equation}
The radiation-pressure net torque is controlled by changing the difference between the light sources' flux $\Delta \Theta(t)$. The maximal tourqe $\tau_{max}$ is the difference is maximal $2 \Theta_{max}$. Thus, with optical setup efficiency $\eta$, the needed flux $\eta\Theta_{max}$ is given by: 
\begin{equation}
