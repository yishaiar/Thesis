...
\par\noindent
The torsional pendulum design, shown in fig.~\ref{fig:pendulum front}, is designed to to balance the achievement of large angle sensitivity to field torque (small string torsion coefficient $\kappa$, eq.~\ref{eqn:theta average}) while maintaining high angle sensitivity to mass (longer oscillation time period $T$, eq.~\ref{eqn:theta average}). 
\par\noindent
For a torsional pendulum with moment of inertia $I$ (eq.~\ref{eqn:moment_inertia}) and string torsion coefficient $\kappa$ (eq.~\ref{eqn:torsion_coefficient_homogeneity}), the period $T$ is given by (eq.~\ref{eqn:undamped_omega}): 
length of $2l$, and two identical masses $m$ on the sides
\begin{equation}
