...
\par\noindent
Since the thermal and Brownian energy arrives by particles with random direction and phase the power can be modelled as a Poisson process with the RMS of the number of gas particles $\sqrt{N}$, resulting with a significantly lower effective power $P_{RMS}$. The noise power is given by:
\par\noindent
\begin{multicols}{2}
\raggedcolumns
\begin{easylist}
