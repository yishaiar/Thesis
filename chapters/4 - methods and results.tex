...
\begin{equation}
W_{PID}(t) = \int_0^T\tau_{PID}(t)\dot{\theta}(t)dt = \int_0^T\gamma\dot{\theta}^2(t)dt = \int_0^T\frac{max(\tau_{PID})}{\theta_{max}}\cdot \frac{ T}{2\pi}\dot{\theta}^2(t)dt = \int_0^T\frac{max(\tau_{PID})}{\theta_{max}}\cdot \frac{ T}{2\pi}\dot{\theta}^2(t)dt = 
\label{eqn:pid work}
\end{equation}
\end{document}