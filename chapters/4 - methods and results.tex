...
\par\noindent
In order to minimize the pressure increase rate, the vacuum chamber is built using CF components which are designed for ultra high vacuum, and minimize the leak rate. Also, the torsional pendulum and vacuum chamber were baked-out to reduce outgassing, for a week at $120 C^0$ using resistive wire. The achieved for the experiment after cooling down is $p=1\cdot 10^{−4} [Hpa] - 5\cdot 10^{−3} [Hpa]$ for a month without pumping with the vacuum engine.
...
\end{equation}the vacuum engine
