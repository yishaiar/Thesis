...
\par\noindent
As shown previously, there are environment noises and friction affecting the torsional pendulum (eq.~\ref{eqn:Brownian power}, eq.~\ref{eqn:heat conduction}, eq.~\ref{eqn:acoustic_intensity}, eq.~\ref{eqn:drag force}, eq.~\ref{eqn: Stefan–Boltzmann power}). The noises power and friction with  gas and environment temperature $T = 298K$, and pressure $P = 1\cdot 10^{−4} [Hpa]$) are:
\begin{easylist}
& Brownian power $p=$
& Thermal power $p=$
& Acoustic power $p=$
