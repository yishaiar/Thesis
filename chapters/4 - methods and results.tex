...
\end{equation}
The PID damping $\gamma$, damping time $\tau$ and damping type $\xi$ all depend on the ratio $\frac{\theta_{max}}{max(\tau_{PID})}$. If the angle is too large compare to the external torque, the PID affect is negligible, causing an infinite damping time with an extremely underdamped pendulum. The damping ratio $\xi$ when the pendulum is critically damped by $\tau_{critical}$ is given by (eq.~\ref{eqn:critically_damped_motion_equation}):
\begin{equation}
