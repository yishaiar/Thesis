...
\begin{equation}
W_T = \int_0^T\tau_{PID}(t)\cdot\dot{\theta}(t) - p_{noise}dt = \int_0^T\gamma\dot{\theta}\cdot\dot{\theta}(t)dt-\int_0^T p_{noise}dt 
\label{eqn:pid work}
\end{equation}
\begin{equation}
W_T= \int_0^T\frac{max(\tau_{PID})}{\theta_{max}} \frac{ T}{2\pi}(\frac{2\pi}{T}\theta_{max})^2 sin(\frac{2\pi}{T}t)dt  
\label{eqn:pid work}
\end{equation}
\begin{equation}
W_{PID} = \frac{\tau_{max}\theta_{max}2\pi}{T} \frac{T}{2} = \theta_{max}\cdot\pi\tau_{max}
\label{eqn:pid work}
\end{equation}
\begin{equation}
Q =2\pi\cdot\frac{energy stored}{energy dissipated per cycle}=2\pi\cdot \frac{U}{W_T}= 2\pi\cdot\frac{0.5\kappa \theta_{max}^2}{\theta_{max}\cdot\pi\tau_{max}} \approx \frac{1.3\cdot 10^{-6} \theta_{max}}{1.0\cdot 10^{-11}}\approx 1.3\cdot 10^{5}\cdot \theta_{max}
\label{eqn:pid work}
\end{equation}
\end{document}