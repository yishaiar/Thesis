...
\par\noindent
The Arduino Mega 2560, is based on the ATmega2560 8-bit controller, and equipped with a 16 MHz crystal oscillator (clock) and 15 PWM outputs pins. Arduino doesn't have analog output, but is simulates analog output using Pulse Width Modulation (PWM).
\subsubsection{Pulse Width Modulation}
\par\noindent
The controller switches the output signal fast between the board's digital output $Vcc = 5[V]$ and off, generating a square wave. Repeating the pattern fast enough (having a short period $T$ to the square wave) results in an analog signal almost as if the signal is a steady analog voltage. The time signal duration is called the pulse width ($PW$). With the clock the controller is able to modulate the $PW$, changing the ratio of the time signal is on compared to off, which is called duty cycle $D$:
\begin{equation}
D = \frac{PW}{T}\cdot 100  \label{eqn:duty cycle}
\end{equation}
The analog voltage value is the average voltage, determined by the duty cycle:
\begin{equation}
V = \frac{\sum PW\cdot Vcc}{\sum T} = \frac{N\cdot PW\cdot Vcc}{N\cdot T} = \frac{Vcc}{100}\cdot D   \label{eqn:analog voltage}
\end{equation}
The Arduino is able to modulate the duty cycle, with PWM Frequency of up to 500Hz, resulting in modulation to the analog signal:
\begin{equation}
V(t) = \frac{Vcc}{100}\cdot D(t)   \label{eqn:analog voltage}
\end{equation}
The method enables generating voltages between $Vcc = 5[V]$ and off, the voltage modulation is limited by the microcontroller resolution (8-bit). Also, since the Arduino clock is connected to all PWM pins, they are all in sync. When all pins have the same duty cycle, they all have the same $V(t)$. At every point in time each could be looked at as a constant voltage supply compare to the other pins, allowing to connect all in series and increase the total output current up to $1[A]$. 
