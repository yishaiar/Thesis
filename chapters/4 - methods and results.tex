...
\end{equation}
In order to critically damp the torsional pendulum by the PID, the external torque $\tau_{PID}$ needs to be able to exert torques as large as the critical damping torque $\tau_{critical}$. The PID external torque $\tau_{PID}$ is given by:
\begin{equation}
max(\tau_{PID}) \geq  \tau_{critical} \geq \frac{ 8 \pi^2 I }{T^2}\cdot\theta_{max} \geq \frac{ 8 \pi^2 \cdot 0.487\cdot10^{-3} }{86^2}\cdot 1\cdot10^{-6}\geq
\label{eqn:damping_ratio_pid}
\end{equation}
\par\noindent
