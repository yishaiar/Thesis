...
\label{eqn:Q}
\end{equation}
The damping would have a non negligible effect on the motion when the oscillator is not highly underdamped $Q < 1$. It can be seen from eq.~\ref{eqn:Q} that $Q$ is highly depended on the amplitude size, even when assuming no coupled noises ($p=0$), the amplitudes with which the damping affects the motion are limited to being smaller than:  
\begin{equation}
