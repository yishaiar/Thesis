...
At each of the small viewports on the sides of the chamber there is a controlled LED light source, which are used to generate torque to damp the system using a PID algorithm.
...
\end{equation}
The duty cycle varies between 0 to 100\%, with the microcontroller resolution. The analog voltage $V_a$ is the average voltage, determined by the duty cycle. The Arduino is able to modulate the duty cycle, resulting in modulation of the voltage:
\begin{equation}
V_a(t) = \frac{ PW(t)\cdot V_d}{ T}  = \frac{V_d}{100}\cdot D(t)  = \frac{5}{100}\cdot D(t)  \label{eqn:analog voltage}
\end{equation}
The method enables generating voltages between $V_d = 5[V]$ and off, the voltage resolution is limited by the microcontroller resolution (8-bit). Also, since the Arduino clock is connected to all PWM pins, they are all in sync. When all PWM pins have the same duty cycle, they all have the same voltage, frequency and phase. Even though the pins output varies in time, they are constant voltage supplies compare to each other, allowing to connect $N$ pins in parallel and increase the output current. 
\begin{equation}
I_{total} = N\cdot I   \label{eqn:analog current}
\end{equation}
...
The setup is composed by two Arduino controlled LED, which are driven by arduino PWM pins. 


Since the forward voltage varies as the logarithm of the current, it varies slowly, being approximately constant over wide current range. Having a constant forward voltage results with large changes in the LED current due to small changes in the driving circuit supply voltage $V$. LED driving circuit consisted of resistor $R$ in series with the LED stabilizes the current, the circuit current is given by:
\begin{equation}
I =\frac{V-V_l}{R} \label{eqn:led circuit}
\end{equation}
The circuit is composed by a blue LED with $V_l\approx 4.5V$, 6 PWM Arduino pins in parallel supply voltage and a resistor $R = 200\Omega$, the LED output power is given by:
\begin{equation}
...
\end{equation}
The LED power could be approximated by linear approximation of Arduino duty cycle $D(t)$, which varies between 0 to 100\% with 8bit resolution, giving a maximal power output of approximately %P = 670mW%. As shown in eq.~\ref{eqn:pwm frequency}), the LED could be modulated up to %500[Hz% resulting with fast modulated light source with high intensity. Since the led power could be approximated using linear approximation, it allows to control the approximated power output by changing the PID gain values.  
\par\noindent
In order to overcome the led uncoherent profile and large opening angle, light was transferred through a light guide. Light guides are pipes made of thin filaments causing internal reflections used for directed transmission of luminous energy. Light guides are used to illuminate small areas, regardless of the spectral characteristics of the light source. They mainly depend on the cross sections of entrance and exit and the length. Making them ideal to overcoming focusing problems. The resulting torque is given by:
\begin{equation}
\tau \approx l(F_1 - F_2) \approx \frac{2l\eta}{{c}} (\Theta_1 -\Theta_2) 
