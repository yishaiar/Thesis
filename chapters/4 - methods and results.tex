...
\par\noindent
In order to balance the pendulum so that the mirror will not have an initial downward tilt, the pendulum center of mass needs to be adjusted. 
\par\noindent
The center of mass is adjusted using a matching mass of the mirror from behind, which balances the mirror weight.
\par\noindent
As shown in fig.~\ref{fig:pendulum top}, matching mass $m_1$ has a similar weight and shape as the mirror $m_2$, and both are connected to the pendulum via spring $w$ from their center. The spring enables accurate distance adjusting to prevent downward tilt.
\begin{figure}[htbp]
	\centering
