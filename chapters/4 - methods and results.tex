...
\end{equation}
The coupled noise coupled to the torsional pendulum is composed of Brownian motion, acoustic waves and black body radiation. When the environment is with out distinguishable acoustic noises at low pressure with room temperature, the coupled noise is mainly due to Brownian motion $p_{eff}\approx 4.4\cdot 10^{-25}  [W]$. Thus, in the right conditions it might be possible to reach a damped amplitude of $\theta_{max}^{PID}\approx 2\cdot 10^{-14}$ (achieved from calculating the Brownian noise effect on the Q factor).
