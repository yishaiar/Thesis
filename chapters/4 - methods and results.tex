...
\end{equation}
Where $\gamma$ is the damping coefficient. The PID is controlling the LED radiation force, given in eq.~\ref{eqn:led torque}, where $D$ the duty cycle varies between 0 to 100%:
\begin{equation}
...
\end{equation}
\par\noindent
The PID damping $\gamma$ and damping time $\tau$ depend on the initial oscillations angle. If the angle is too large compare to the external torque, the system is extremely underdamped and the PID affect is negligible, causing an infinite damping time, and pendulum would keep oscillating.
