...
\end{equation}
The PID controller continuously calculates the oscillator's error value, damping it to zero. The PID acts as friction, gradually working when the oscillations are at the maximum velocity to slow them down, and remove the torque energy:
\begin{equation}
\tau_{PID} = -\gamma\dot{e}(t) =  -\gamma(\dot{\theta}(t)-0)  =  -\gamma\dot{\theta}(t)  \label{eqn:friction_torque_pid}
\end{equation}
Where $\gamma$ is the damping coefficient, if the defined set point zero, the error is the measured process variable $\theta(t)$. 
The PID is controlling the LED radiation force, given in eq.~\ref{eqn:led torque}, where $D$ the duty cycle of each LED varies between 0 to 100%:
\begin{equation}
