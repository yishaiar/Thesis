...
\end{equation}
\begin{equation}
U = \frac{\kappa\theta(t)^2}{2}+\frac{I\dot{\theta}(t)^2}{2} = \frac{\kappa\theta(t)^2}{2}+\frac{I\dot{\theta}{t}^2}{2}
\label{eqn:pendulum energy}
\end{equation}

\begin{equation}
Q =2\pi\cdot\frac{maximum\ energy\ stored}{energy\ dissipated\ per\ cycle}=2\pi\cdot \frac{U}{W_T}= 2\pi\cdot\frac{0.5\kappa \theta_{max}^2}{\frac{\theta_{max}\cdot\pi\tau_{max}\cdot T -p_{noise}}{T}} \approx \frac{1.3\cdot 10^{-6} \theta_{max}}{1.0\cdot 10^{-11}}\approx 1.3\cdot 10^{5}\cdot \theta_{max}
\label{eqn:pid work}
\end{equation}
\end{document}