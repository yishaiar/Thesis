...
As shown in eq.~\ref{eqn:theta average} the gravimetric measurement is conducted by measuring the equilibrium angle of the torsional pendulum. The measurement is limited by the quantum uncertainty due to thermal energy (eq.~\ref{eqn:Brownian uncertainty 3}). When the torsional pendulum is damped beyond the room temperature quantum uncertainty, the PID is effectively cooling down the torsional pendulum temperature and thus reducing the measurement uncertainty. The gravimetric measurement is given by:
\begin{equation}
...
\end{equation}
Assuming a measured mass $M$ at distance $r_0 = 1[m]$, the room temperature measurement sensitivity is limited to 
