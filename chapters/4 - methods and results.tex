...
\end{equation}
The radiation-pressure net torque is modulated by changing the difference between the light sources' flux (enabling high dynamic range to modulation). 
\begin{equation}
max(\tau)  \approx \frac{2l\eta}{{c}} \cdot 2 \Theta_{max} \approx \frac{0.218\cdot 0.7}{{3\cdot10^{8}}} \cdot 2 \Theta_{max} \approx 5.1\cdot10^{-10} \cdot \Theta_{max}   \label{eqn:max radiation torque}
\end{equation}
As shown in eq.~\ref{eqn:damping_torque_pid}, to damp the torsional pendulum the light sources maximal flux is given by:
\begin{equation}
max(\tau)  = \frac{2l\eta}{{c}} \cdot 2 \Theta_{max} \approx \frac{0.218\cdot 0.7}{{3\cdot10^{8}}} \cdot 2 \Theta_{max} \approx 5.1\cdot10^{-10} \cdot \Theta_{max} \geq 2.6\cdot10^{-11}  \label{eqn:max radiation torque}
\end{equation}
