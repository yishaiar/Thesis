...
\begin{equation}
Q = 2\pi\cdot \frac{U}{W_T}=  \frac{2\pi\cdot(\frac{\kappa\cdot\theta(t)^2}{2}+\frac{I\cdot\dot{\theta}(t)^2}{2})}{\int_0^T[\tau_{PID}(t)\cdot\dot{\theta}(t) - p]dt} = \frac{\pi\cdot(\kappa\theta_{max}^2cos^2(\frac{2\pi}{T}\cdot t)+I\omega^2\theta_{max}^2sin^2(\frac{2\pi}{T}\cdot t) }{\int_0^T[\frac{\tau_{max} }{\theta_{max}} \frac{ T}{2\pi}(\frac{2\pi}{T}\theta_{max})^2 sin(\frac{2\pi}{T}t)dt-p] dt }
\label{eqn:Q}
\end{equation}
...
\begin{equation}
Q(\theta_{max},p) =  \frac{\pi\cdot(\kappa\theta_{max}^2cos^2(\omega\cdot 0)+I\omega^2\theta_{max}^2sin^2(\omega\cdot 0) }{\frac{\tau_{max} }{\theta_{max}} \frac{ T}{2\pi}(\frac{2\pi}{T}\theta_{max})^2\int_0^T sin(\frac{2\pi}{T}t)dt-p\int_0^T dt }  =  \frac{\pi\kappa\theta_{max}^2}{T\cdot(\frac{\theta_{max}\pi\tau_{max}}{T} -p)} 
\label{eqn:Q}
\end{equation}
