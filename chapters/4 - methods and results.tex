...
\end{equation}
\par\noindent
In order to work with as thin string as possible, tungsten string with high tensile strength was chosen. The high tensile strength enables to overcome suction forces while vacuum engine is working and holding the pendulum without tear. Tungsten is also vacuum compatible. Tungsten string info;
\begin{itemize}
	\item length of 249 mm
	\item diameter of 0.05 mm
	\item 99\% pure tungsten 	
\end{itemize}
Torsional beam dimensions;
\begin{itemize}
	\item length of 238 mm
	%\item width of 10 mm 
	%\item weight of 90 gram 
	\item masses weight 20.5 gram each
\end{itemize}
The experiment, is constructed using a front mirror, the angle displacement causes a mirror tilt. Mirror;
\begin{itemize}
	\item OFC copper mirrror, gold coated
	\item low out gassing (although officially not vacuum compatible) 
	\item 1 inch diameter	
\end{itemize}
 is a  , with .
