...
\par\noindent
In order to balance the pendulum so that the mirror will not have an initial downward tilt due to earth gravity $g$. The pendulum center of mass needs to be adjusted. With a balanced pendulum the downward torques would be canceled out $\tau_y = 0$. The center of mass is adjusted using a balancing mass $m_1$ to the mirror weight $m_2$ from behind:
\begin{equation}
...
\end{equation}
\par\noindent
As shown in fig.~\ref{fig:pendulum top}, the balancing mass has a similar shape and weight as the mirror, and both are connected to the pendulum via spring $w$ from their center. The spring enables accurate distance adjusting to prevent the downward tilt.
\begin{figure}[htbp]
	\centering
...
\FloatBarrier
\par\noindent
The upper tube of the chamber is soldered with a base to which a mount is connected. The pendulum string is held via the mount. The mount is adjustable, enabling the adjustment of the pendulum's string length, to be in front of viewports accurately. 
...
\end{equation}
In this experiment, due to the relatively large size and unique shape of the measurement equipment, the vacuum chamber has a significant leak and outgassing rates. The pumping must be disconnected during measurement due to the measurement sensitivity, causing pressure increase over time.
...
\par\noindent
As demonstrated previously (eq.~\ref{eqn:total_kinetic}, eq.~\ref{eqn:acoustic_intensity}, eq.~\ref{eqn:drag force}), Brownian motion, acoustic waves and friction are pressure dependent and are further reduced by maintaining low pressure (through use of vacuum).
