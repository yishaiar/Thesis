...
\label{eqn:Q}
\end{equation}
The damping would effect the oscillations when the oscillator is not highly underdamped $Q < 1$. It can be seen from eq.~\ref{eqn:Q} that $Q$ is highly depended on the amplitude size $\theta_{max}$. Even when assuming no environmental noises $p=0$, the amplitudes at which $Q < 1$ are:  
\begin{equation}
...
\end{figure} 
 ...
 When $Q = 0.5$ the oscillator is critically damped, and when $Q < 0.5$ there is over damping. When Q<0 the noise power coupling is larger than the work done by the PID at the cycle, causing the oscillator to be driven. 
\par\noindent
