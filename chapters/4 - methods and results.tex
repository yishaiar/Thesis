...
\begin{figure}[htbp]
	...
	\caption[The PWM analog voltage]{The PWM analog voltage of a single pin}
...
\end{equation}
The Arduino is able to modulate the duty cycle in time, resulting in modulation of the average voltage $V_a$ which is the simulated analog voltage of a single pin. Also, since the Arduino clock is connected to all PWM pins, they are all in sync. When all pins have the same duty cycle $D$, they all have the same voltage frequency and phase, resulting with the pins being constant voltage supplies compare to each other. When connecting all pins in series, the total output $V$ power is given by:
\begin{equation}
V(t) = N\cdot V_a(t) = N\cdot\frac{ PW(t)\cdot V_d}{ T}  = N\cdot\frac{V_d}{100}\cdot D(t)   \label{eqn:analog voltage}
\end{equation}
...
\end{equation}
Where $I_s$ is the saturation current of the diode, $V_d$ is the LED forward voltage, $n$ is the led emission coefficient, $e$ is the electron charge, and $V_T$ is the thermal voltage. The LED forward voltage at room temperature ($T=300K$) is given by:
\begin{equation}
V_d \approx n V_T ln10 log_{10} (\frac{I}{I_s}) \label{eqn:led voltage}
\end{equation}
Since the forward voltage is logarithm of the current, it varies very slow and is approximately constant over a wide current range. Having a constant forward voltage results with large changes in the LED current due to small changes in the driving circuit supply voltage. LED driving circuit consisted of a resistor in series with the LED stabilizes the current, the circuit current is given by:
\begin{equation}
I =\frac{V-V_d}{R} \label{eqn:led circuit}
\end{equation}
The LED power is given by:
\begin{equation}
P = I =\frac{V-V_d}{R} \label{eqn:led circuit}
\end{equation}
