...
A PID feed-back loop is damping the torsional pendulum by continuously calculating the oscillator's error value $e(t)$ from a defined set point $SP$, and damping it to zero.
...
\end{equation}
Where $\gamma$ is the PID damping coefficient, since the set point is constant, the error derivative $\dot{e}(t)$ is the measured process velosity which is the angular velosity of the torsional pendulum $\dot{\theta}(t)$. 
\par\noindent
