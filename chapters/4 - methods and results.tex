...
\par\noindent
The apparatus uncertainty due to the non linearity of the AOM response and laser power fluctuations proved to be larger than the uncertainties of the intensity difference. Also, modulation speed (response time) proved to be more important than modulation range. Due to the conclusions, the PID torque was changed to two controlled high power LED sources. 
\subsection{Light emitting diode (LED)}
LED sources having a linear response of the torque (see eq.~\ref{eqn:radiation torque}) which minimizes the overshoot, and a fast response time which minimizes phase delay.
...
\end{equation}
Where $I$ is the LED current, $I_s$ is the saturation current, $n$ is the emission coefficient, and $V_T$ is the thermal voltage. Since the forward voltage varies as the logarithm of the current, it varies slowly, being approximately constant over wide current range, resulting with large changes in the LED current due to small changes in the circuit supply voltage $V$. In the driving circuit a resistor $R$ is connected in series with the LED to stabilizes the current, the current $I$ of a circuit with $N$ voltages supplies in parallel is given by:
\begin{equation}
I =\frac{N\cdot V-V_l}{R} \label{eqn:led circuit}
\end{equation}
...
\begin{equation}
\Theta(t) = I\cdot V_l = I\cdot V_l =  V_l \frac{N\cdot V-V_l}{R}  = V_l\frac{N\cdot\frac{V_d}{100}\cdot D(t)-V_l}{R} \approx \frac{N V_lV_d}{100R}\cdot D(t) \label{eqn:led power}
\end{equation}
\par\noindent
