...
\par\noindent
The controller switches the output signal fast between the digital output and off, generating a square wave. The controller is repeating the pattern generating a square wave with a period of ) results in an analog signal almost as if the signal is a steady analog voltage. The time signal duration is called the pulse width ($PW$). With the clock, the controller is able to modulate $PW$, changing the ratio of the time signal is on compared to off, which is the duty cycle $D$:
\begin{equation}
