...
\label{eqn:net power}
\end{equation}
Due to Brownian motion, $n_p= \frac{N}{V}v A$, gas particles with mass $m_0 = \frac{M}{N_A}$ and average velosity of gas flow $v$ (eq.~\ref{eqn:flow velocity}) collide into $A$. Assuming all collisions are elastic, each particle exchanges momentum of $ k = 2 m_0 v$. Thus, causing Brownian power $p$, which is given by:
\begin{equation}
p =  n_p\frac{ k^2}{m_1} =  \frac{N}{V} v A\frac{ (2 m_0 v)^2}{m_1} = \frac{4AN }{m_1V}(\frac{M}{N_A})^2 (\frac{3 k_B N_A T}{M})^{3/2}
\label{eqn:net power}
\end{equation}
