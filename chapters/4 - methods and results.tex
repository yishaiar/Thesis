...
\FloatBarrier
\par\noindent
The experiment setup, shown in fig.~\ref{fig:setup}, is composed by the torsion pendulum, a measurement system and a feedback system all placed inside a seismic box. The tilt angle is measured by a LASER beam tilted by the pendulum's front mirror and measured by a quadrant detector, the detector is connected to a computer by an oscilloscope through a signal amplifier. The tilt angle signal is passing by a piezo driven mirror, which is tuned making sure the pendulum is balanced relative to the detector's center (the PID needs a reference for error calculations, otherwise it would keep a string torsion and would fail in damping). The feedback system is composed by two LED light sources, modulated in real time by an Arduino micro-controller which is controlled by a PID feed-back loop in the computer. The LED light sources are placed in front of each of the viewports on the sides of the chamber, each coupled to a side mass of the pendulum. 
\par\noindent
When the desired vacuum is achieved, the vacuum engine is disconnected using the valve and turned off, and the seismic box is closed. Approximately after eight hours the pendulum's oscillations settle down to the amplitude caused by the environment noises level and which could be affected by the weak torques caused by radiation pressure.  The angle is read on real time by the algorithm, and an equivalent radiation-pressure torque is exerted by the LED's flux. 
