...
\end{equation}
Where $\gamma$ is the PID damping coefficient. When the defined set point is zero, the error $e(t)$ is the measured process which is the tilt angle of the torsional pendulum $\theta(t)$. The torque is modulated proportionately to the velocity of the oscillations, meaning it needs to have high speed and high dynamic range modulations.
\par\noindent
The torsional pendulum is a simple harmonic oscillator with maximal velocity $\dot{\theta}_{max} = \theta_{max}\cdot\frac{2\pi}{T} $ (eq.~\ref{eqn:undamped_motion_equation_solved}). The PID damping coefficient $\gamma$ is a constant value given by:
\begin{equation}
...
\end{equation}
The PID controlled torque $\tau_{PID}(t)$ needs to have high modulation dynamic range and speed to continuously damp the oscillations (eq.~\ref{eqn:friction_torque_pid}), and a maximal torque given by eq.~\ref{eqn:damping_torque_pid} to achieve critical damping.
\par\noindent
