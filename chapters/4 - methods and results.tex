...
Due to random motion, we estimate $n_p= \frac{N}{V}v A$ gas particles collisions per second into the cross section of the pendulum. Each particle exchanges momentum $k$, resulting with force $F = n_{col}\cdot k$. Due to the collisions the torsional pendulum velosity is $ u = n_{col}\frac{k}{m_1}$. Resulting with power given by:
\begin{equation}
