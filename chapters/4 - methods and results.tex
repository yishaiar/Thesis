...
\par\noindent
As shown previously, there are other environment noises affecting the torsional pendulum (eq.~\ref{eqn:Brownian power}, eq.~\ref{eqn:heat conduction}, eq.~\ref{eqn:acoustic power}, eq.~\ref{eqn: Stefan–Boltzmann power}). Assuming gas and environment temperature of $T = 298K$ and pendulum temperature $T_0 = 0K$ (maximal noise power), the maximal power of the noises are given by:
\begin{easylist}
...
\end{easylist}
Although the noise power can be very large, since it arrives with random direction it can be modelled as a Poisson process with the RMS of the number of gas particles, resulting with an effective pressure dependency of the power. The  effective pressure $P_{RMS}$  is given by:
\begin{equation}
P_{RMS} =\sqrt{N}\frac{k_B T}{V}=\sqrt{P}\cdot \sqrt\frac{k_B T}{V}  \kappa\theta    \label{eqn:P_RMS}
\end{equation}
