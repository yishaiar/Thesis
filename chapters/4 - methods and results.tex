...
\par\noindent
The controller switches the output signal fast between the digital output and off, generating a square wave. The controller is repeating the pattern generating a square wave with a period of $T \ propto 2\cdot 10^{-6}$) results in an analog signal almost as if the signal is a steady analog voltage. The time signal duration is called the pulse width ($PW$). With the clock, the controller is able to modulate $PW$, changing the ratio of the time signal is on compared to off, which is the duty cycle $D$:
\begin{equation}
...
\end{equation}
The Arduino is able to modulate the duty cycle, resulting in modulation to the analog voltage:
\begin{equation}
V_a(t) = \frac{V_d}{100}\cdot D(t)   \label{eqn:analog voltage}
\end{equation}
The method enables generating voltages between $V_d = 5[V]$ and off, the voltage resolution is limited by the microcontroller resolution (8-bit). The possible modulation speed is up to the PWM Frequency of 500Hz.
\par\noindent 
