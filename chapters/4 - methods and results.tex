...
& Thermal flow;
&& $p=0.083[W]$.
&& $p_{RMS}=2.4\cdot 10^{-9}[W]$.
\end{easylist}
\columnbreak
\begin{easylist}
& Brownian motion;
&& $p=0.346[W]$.
&& $p_{RMS}=1\cdot 10^{-8}[W]$.
\end{easylist}
\end{multicols}
The black body radiation is emitted by photons, although we don't have an accurate estimation of the RMS, we estimate a similar reduction of the effective power.    
\begin{easylist}
...
&& $p=1.14\cdot 10^{-28}[W]$.
\end{easylist}
The measurement system is placed inside a acoustic box,further reducing the acoustic power. The acoustic box is also a Faraday cage with 76 mm thickness, blocking magnetic fields with frequencies $f \ge 30 [Hz]$ from the environment.
\par\noindent
