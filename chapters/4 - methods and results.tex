...
\section{Noise}
As shown previously, there are multiple noise sources at the environment(eq.~\ref{eqn:Brownian power}, eq.~\ref{eqn:heat conduction}, eq.~\ref{eqn:acoustic power}, eq.~\ref{eqn: Stefan–Boltzmann power}). Assuming gas and environment temperature of $T = 298K$ and pendulum temperature $T_0 = 0K$ (maximal noise power), The noise power is given by:
\par\noindent
\begin{multicols}{2}
\raggedcolumns
\begin{easylist}
