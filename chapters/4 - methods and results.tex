...
\begin{equation}
I  = I_s (e^{V_d\cdot \frac{e}{n k_B T} }-1) \approx^{V_d>>V_T} I_s e^{\frac{V_d}{n V_T} }  \label{eqn:led current}
\end{equation}
Where $I_s$ is the saturation current of the diode, $V_d$ is the LED forward voltage, $n$ is the led emission coefficient, $e$ is the electron charge, and $V_T$ is the thermal voltage. When $V_d>>V_T$ the exponent is mich is given by:
\begin{equation}
