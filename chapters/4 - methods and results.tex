...
\FloatBarrier
\par\noindent
The experiment system, shown in fig.~\ref{fig:setup}, is composed by the torsion pendulum, a measurement system and a feedback system. The tilt angle measurement is done by a quadrant detector, which connected to the computer by an oscilloscope through an amplifier. The feedback system is composed by two LED light sources, modulated in real time by an Arduino micro-controller which is controlled by a PID feed-back loop. The angle is read on real time by the algorithm, and an equivalent external torque is coupled to the pendulum by the LED flux. 
