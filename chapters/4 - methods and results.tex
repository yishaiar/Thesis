...
\end{equation}
The coupled noise coupled to the torsional pendulum is composed of Brownian motion, acoustic waves and black body radiation. When the environment is at low pressure and temperature and with out distinguishable acoustic noise, the main noise is due to Brownian motion $p_{eff}\approx 4.4\cdot 10^{-25}  [W]$. Thus, with a possible damped angle of $\theta_{max}^{PID}\approx 2\cdot 10^{-14}$ (achieved from calculating the noise effect on the Q factor).
