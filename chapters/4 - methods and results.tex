...
\begin{equation}
\tau_{max}  \approx \frac{2l\eta}{{c}} \cdot 2 \Theta_{max} \approx \frac{0.218\cdot 0.7}{{3\cdot10^{8}}} \cdot 2 \Theta_{max} \approx 3.63\cdot10^{-10} \cdot \Theta_{max}   \label{eqn:max radiation torque}
\end{equation}
Where $\Theta_{max}$ is the is the the light sources' maximal flux and the assumed efficiency of $\eta \approx 0.5$. As shown in eq.~\ref{eqn:damping_torque_pid}, to successfully damp the torsional pendulum the maximal flux $\Theta_{max}$ is given by:
\begin{equation}
\Theta_{max} \geq \frac{2.6\cdot10^{-11}}{3.63\cdot10^{-10}} \geq 72 mW 
 \label{eqn:max radiation flux}
\end{equation}
\subsection{Laser setup}
Initially the modulated light sources were composed of a single high power laser diode coupled in series into two acousto optic modulators (AOM), with mod cleaning by coupling the modulated light into the torsional pendulum through optical fibers. An AOM can divide a single coherent light beam into two beams, with intensity ratio proportional to the AOM response to input voltage, resulting with a controlled intensity ratio between the beams.
\par\noindent
