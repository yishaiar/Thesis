...
\begin{equation}
\xi = \frac{T}{2 \pi \tau } =  \frac{2I}{\gamma} = \frac{\theta_{max}}{ \tau_{max}} \cdot \frac{4\pi I}{T}  \label{eqn:damping_time_pid}
\end{equation}
...
\par\noindent
In order to succeed in damping the pendulum, the PID must be able to generate torques which can critically damp the equilibrium amplitude (eq.~\ref{eqn:critically_damped_motion_equation}). When the pendulum is critically damped  by $\tau_{critical}$, the damping ratio $\xi$ is given by :
\begin{equation}
