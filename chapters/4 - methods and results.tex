...
\par\noindent
The controller switches the output signal fast between the board's digital output $Vcc = 5[V]$ and off, generating a square wave. Repeating the pattern fast enough results in an analog signal almost as if the signal is a steady analog voltage. The time signal duration is called the pulse width. With the clock controller is able to modulate the pulse width, changing the ratio of the time signal is on compared to off. 
\par\noindent
