...
The energy lost in one cycle is the work done at the cycle, which is the difference between the PID work and the environment noise power $p_{noise}$ coupled to the oscillator at the cycle, the work $W_T$ is given by:
\begin{equation}
W_T = \int_0^T[\tau_{PID}(t)\cdot\dot{\theta}(t) - p_{noise}]dt = \int_0^T\frac{max(\tau_{PID})}{\theta_{max}} \frac{ T}{2\pi}(\frac{2\pi}{T}\theta_{max})^2 sin(\frac{2\pi}{T}t)dt-p_{noise}\int_0^T dt 
...
\begin{equation}
Q = 2\pi\cdot \frac{U_T}{W_T}= 2\pi\cdot\frac{\frac{\kappa\theta_{max}^2}{2}}{T\cdot(\frac{\theta_{max}\cdot\pi\tau_{max}}{T} -p_{noise})} =
...
\label{eqn:pid work}
\end{equation}
When $Q>0.5$ the system is underdamped, when the value is very large the system is highly underdamped. When $Q = 0.5$ the system is critically damped, when $Q < 0.5$ the system is over damped (in reality the PID is tunable, allowing to produce lower torques than the maximal, keeping the system critically damped). When Q<0 the noise power IS larger than the work done by the PID causing the system to be driven. 
