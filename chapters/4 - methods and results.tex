...
\par\noindent
In order to balance the pendulum so that the mirror will not have an initial downward tilt $\tau_y$, the pendulum center of mass needs to be adjusted. 
\par\noindent
...
\end{equation}
\par\noindent
As shown in fig.~\ref{fig:pendulum top}, matching mass $m_1$ has a similar shape and weight as the mirror $m_2$, and both are connected to the pendulum via spring $w$ from their center. The spring enables accurate distance adjusting to prevent the downward tilt.
\begin{figure}[htbp]
	\centering
