...
Two light sources with a given flux causes a radiation-pressure force (eq.~\ref{eqn:radiation_force_power}), one on each mass of the torsional pendulum. As seen in eq.~\ref{eqn:net_gravitation_torque}, the difference between the two torques, add an external net torque. Assuming that both light fields have the same coupling efficiency $\eta$ and are very close to being perpendicular to surface, causing both incidence angles to be negligible $\alpha_1\approx\alpha_2\approx 0$. The radiation-pressure net torque is given by:  
\begin{equation}
...
\end{equation}
Where $\alpha$ is the force angle compared to the pendulum surface.  
\begin{equation}
