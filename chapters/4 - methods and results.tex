...
A PID feed-back loop is damping the torsional pendulum by continuously calculating the error value of the measured signal from a defined set point $SP$, and damp it to zero  $e(t) =\theta(t)-SP = 0 $ . The control system aims to reach $SP$ by critically damping of the process (the torsional pendulum). A well tuned PID reaches the $SP$ fast and accurate, and able to resist forces trying to move the process from the $SP$. 
\par\noindent
In control theory there are two wanted conflicting properties; an accurate response (small overshoot), and small risetime (fast response). Overshoot is when output signal or function exceeds the target value, thus the response signal is not accurate compare to target.  Overshoot is usually measured in percentage overshoot (PO). For second order systems, such as damped oscillators PO is given by:
\begin{equation}
