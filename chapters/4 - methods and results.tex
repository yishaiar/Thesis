...
\end{equation}
\par\noindent
Where $\gamma$ is the PID damping coefficient, since $SP$ is constant, $\dot{e}(t)$ is the the torsional pendulum's oscillations velosity $\dot{\theta}(t)$. The torque is proportional to the oscillations velocity, meaning it needs a high speed modulation compare to the oscillations period $T$ (to prevent phase delay), and high dynamic range compare to oscillations maximal velosity $\dot{\theta}_{max}$ (to avoid overshoot). 
\par\noindent
The torsional pendulum is a simple harmonic oscillator with maximal velocity $\dot{\theta}_{max} =\frac{2\pi}{T} \theta_{max}$ (see eq.~\ref{eqn:undamped_motion_equation_solved}) and initial oscillations angle $\theta_{max}$. The PID damping coefficient $\gamma$ is a constant value given by:
\begin{equation}
...
\end{equation}
The PID controlled torque $\tau_{PID}(t)$ needs a maximal torque given by eq.~\ref{eqn:damping_torque_pid} and modulations with high speed and high dynamic range for continuous damping of the oscillations (eq.~\ref{eqn:friction_torque_pid}), where the sufficient modulation speed and range are unknown. 
\par\noindent
