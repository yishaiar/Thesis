...
\end{equation}
The radiation-pressure net torque can be controlled by changing the difference between the light sources' flux $\Delta \Theta(t)$. With maximal light source flux of $Theta_{max}$, the maximal difference is $2 \Theta_{max}$. Thus, the needed flux $\eta\Theta_{max}$ is given by: 
\begin{equation}
\eta\Theta_{max} \geq \frac{\tau_{max}}{\frac{c}{4l}}\approx
...
Assuming negligible friction, the energy lost in one cycle is the work done at the cycle, which is the difference between the PID work and the environment noise power $p$ coupled to the oscillator at the cycle (which is independent of time). The torsional pendulum Quality factor is:
\begin{equation}
Q = 2\pi\cdot \frac{U_T}{W_T}= 2\pi\cdot \frac{\frac{\kappa\cdot\theta(t)^2}{2}+\frac{I\cdot\dot{\theta}(t)^2}{2}}{\int_0^T[\tau_{PID}(t)\cdot\dot{\theta}(t) - p]dt} = 2\pi\cdot \frac{\frac{\kappa(\theta_{max}cos(\omega\cdot 0))^2}{2}+\frac{I(-\omega\theta_{max}sin(\omega\cdot 0))^2}{2} }{\int_0^T\frac{\tau_{max} }{\theta_{max}} \frac{ T}{2\pi}(\frac{2\pi}{T}\theta_{max})^2 sin(\frac{2\pi}{T}t)dt-p\int_0^T dt }
\label{eqn:Q}
\end{equation}
\iffalse
\begin{equation}
...
\begin{figure}[htbp]
	\centering
	\fbox{\includegraphics[scale=0.3]{\main/images/4 - methods and results/Q factor.png}}
