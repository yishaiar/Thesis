...
\end{equation}
The circuit is composed by a blue LED with $V_l\approx 4.5V$, supply voltage of 6 PWM Arduino pins in parallel and resistor $R = 200\Omega$, the LED output power is given by:
\begin{equation}
...
\end{equation}
\par\noindent
The LED power could be approximated by linear approximation of the Arduino duty cycle $D(t)$, which varies between 0 to 100\% with 8bit resolution, giving a maximal power output of approximately $P = 670mW$ and torques up to $\tau = 7\cdot10^-{10} [N\cdot m]$ with modulation steps of $\Delta\tau = 3.5\cdot10^-{12} [N\cdot m]$. As shown in eq.~\ref{eqn:pwm frequency}), the LED could be modulated up to $500[Hz]$ resulting with fast modulated light source with high intensity. Since the led power could be approximated using linear approximation, it allows to control the approximated power output by changing the PID gain values.  
