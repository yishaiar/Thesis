...
\FloatBarrier
\par\noindent
The microcontroller can simulate analog output using Pulse Width Modulation (PWM). As shown in fig.~\ref{fig:duty_cycle}, the controller switches the output signal fast between the digital output on and off, generating a square wave with period $T$. Pulse width ($PW$) is the time duration in which the signal is on, the controller is able to modulate $PW$, and thus changing the duty cycle $D(t)$ which is the ratio of time signal is on compared to off $D(t) = \frac{PW(t)}{T}\cdot 100$. The duty cycle varies between 0 to 100, with resolution limited by the controller. The duty cycle modulation results in modulation of the average voltage (the analog voltage) $V_a$ given by: 
\begin{equation}
