...
\end{equation}
\par\noindent
The noise reductance enables to damp and cool the pendulum temperature. Cooling system is built using led light sources modulated at high speeds using a cheap arduino microcontroller. The modulated light is causing a small external radiation torque, which damps down the ossiclations.
\par\noindent
Damping is made using a PID feedback loop. The feedback damping can dynamically damp the noises, without pre-knowledge of the existing noises. PID would reduce all noises, and reduce the temperature. Having an effective lower temperature would enable measuring smaller angles enlarging the SNR and the measurement sensitivity.
