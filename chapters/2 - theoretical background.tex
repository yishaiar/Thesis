...
\par\noindent
An optical signal of $N$ photons with frequency $\nu$ is with power $p_{signal} = N h \nu$, where $h$ is the Planck constant. Since the shot noise is a Poisson process, the shot noise power can be calculated as the root mean square (RMS) of the signal power $p_{shot} = \sqrt{p}$.
\begin{equation}
...
\end{equation}
Where $\beta$ is a numerical constant, $l$ is the mean free path of the gas particles and $m$ is the molecular mass.
\par\noindent
...
\end{equation}
It can be shown from eq.~\ref{eqn:average kinetic}, that the average gas flow velocity $v$ is given by:
\begin{equation}
...
Radiative cooling is a phenomenon in which a black body loses heat by thermal radiation to remain in thermal equilibrium at constant temperature $T_0$. The thermal radiation power $p_{emit}$ over all frequencies is given by the Stefan–Boltzmann law:
\begin{equation}
