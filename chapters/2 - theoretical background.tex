...
\end{equation}
Where $\beta$ is a numerical constant and $l$ is the mean free path of the gas particles. The molecular velocity $u$ of gas particles with mass $m_0$ is given by the Maxwell-Boltzmann distribution, it can be shown that the average kinetic energy of a gas particle $<E_k>$ is given by:
\begin{equation}
<E_k>=<\frac{m_0 u^2}{2}> = \int_{0}^{\infty}\frac{m_0 u^2}{2}f(u)du =  \frac{3k_BT}{2}    \label{eqn:average kinetic}
\end{equation}
\par\noindent
