...
\end{equation}
Where $P$ is the gas pressure, $V$ is the medium's volume, $R$ is the ideal gas constant, $T$ is the temperature and $k_B$ is the Boltzmann constant. The number of particles $N$ is a product of the number of moles $n$ and the Avogadro constant $N_A$. For gas particles with a molar mass $M$ the density $\rho$ is given by:
\begin{equation}
\rho = \frac{MP}{RT}     \label{eqn:ideal density}
\end{equation}
The Reynolds number $Re$ is used to characterize types of gas flow, for an ideal gas $Re$ is given by:
\begin{equation}
Re= \frac{\rho v d}{\mu}     \label{eqn:Reynolds number}
\end{equation}
Where $v$ is the gas flow velocity, $d$ is the diameter of the area cross section and $\mu$ is the fluid viscosity.



\subsection{Brownian motion}
\subsubsection{Brownian energy}
Brownian motion is the pattern of random particles' fluctuations at fluid or gas phases, a random walk with no preferential direction of flow. This pattern happens at thermal equilibrium in a given temperature (on average there is no linear and angular momentum). The Maxwell-Boltzmann distribution of molecular velocity for particles with mass $m$ is:
\begin{equation}
...
There is thermal energy coupling from the environment on the sides of the medium, phonons are penetrating and pass energy to the gas particles inside the medium. The law of heat conduction (Fourier's law) is:
\begin{equation}
\dot{Q} = \lambda A \frac{\Delta T}{\Delta x}   \label{eqn:heat conduction}
\end{equation}
\par\noindent
Where $\dot{Q}$ is the rate of heat flow propagating distance of $\Delta x$ through a material with thermal conductivity $\lambda$ with area $A$.The heat flow is proportional to the temperature difference $\Delta T$. It can be shown that for an ideal gas with a mean free path $l$ the passing distance is $\Delta x ^2  = \frac{1}{3} l^2 $, and the thermal conductivity is:
\begin{equation}
\lambda  = \frac{l}{3}\rho c_v v = \frac{l}{3} \frac{MP}{RT} c_v v   \label{eqn:heat conduction coefficient}
\end{equation}
\par\noindent
Where $c_v$ is the specific heat capacity of the gas, the rate of heat flow is:
\begin{equation}
\dot{Q} =  \frac{l}{3} \frac{MP}{RT} c_v v A \frac{\Delta T}{\sqrt{\frac{1}{3} l^2 }} = \frac{M P c_v v A \Delta T}{\sqrt{3}RT}   \label{eqn:heat conduction 2}
\end{equation}
Eq.~\ref{eqn:heat conduction 2} shows that the rate of thermal energy coupling from the environment is proportional to the gas pressure. Reducing the pressure reduces the energy coupling rate.
...
\end{equation}
It can be shown that the partition function $Z$ for a classical continuous system with one degree of freedom is:
\begin{equation}
Z = \frac{1}{h}\int_{-\infty}^{\infty}dx\int_{-\infty}^{\infty} dp exp(-\frac{p^2}{2 m k_B T}) exp(-\frac{\kappa x^2}{2 k_B T }) = \frac{1}{h} \sqrt{2\pi m k_B T} \sqrt{2\pi k_B T \kappa} = \frac{k_B T}{h\omega} = \frac{1}{\beta h\omega}
\label{eqn:partition function}
\end{equation}
Where $\beta = k_B T$ is the 
The thermodynamic energy of the oscillator is: 
\begin{equation}
E = \frac{1}{2}\kappa (\delta\theta)^2= \frac{1}{2}k_BT  \label{eqn:Brownian uncertainty}
\end{equation}
...
\end{equation}
\par\noindent
Where $P$ is the actual pressure, $P_0$ is the average pressure, $k$ is the acoustic wave number and $\omega$ is the wave angular frequency. The acoustic intensity $I$ is defined by:
\begin{equation}
I = P u      \label{eqn:acoustic_intensity}
\end{equation} 
The acoustic intensity is the power carried by the acoustic waves $[\frac{W}{m^2}]$, and it depends on the material pressure $P$ and the particle velocity $u$. When the material pressure is reduced, the acoustic pressure is lower and less power is carried.
...
Re= \frac{\rho v d}{\mu}     \label{eqn:Reynolds number}
\end{equation}
Where $\rho$ is the gas density, $v$ is the flow velocity, $d$ is the diameter of the area cross section and $\mu$ is the fluid viscosity. The density $\rho$ of an ideal gas: 
\begin{equation}
\rho = \frac{MP}{RT}     \label{eqn:ideal density}
\end{equation}
...
The drag force $F_{drag}$ is the friction caused by gas resistance, which is dependent on the relative velocity (the gas flow velocity) $v$ between the gas and the object. At higher pressures, when Reynolds number is relatively high the drag is proportional to the square of the velocity:
\begin{equation}
...
\end{equation}
%The air viscosity is  $\mu = 1.81/cdot10^{-5}[\frac{kg}{m/cdot s}]$ resulting with a 
