...
\end{equation}
Where the environment black bodies are very close to an ideal black body $ \epsilon_1\approx 1$ at temperature $T$. Eq.~\ref{eqn: Stefan–Boltzmann power} shows that the net absorbed power is depended on the temperature difference from the environment. Additionally, the black body radiation of the environment causes momentum and energy exchange due to photon collisions, similar to the gas particles Brownian motion. The collisions are only depended on the temperature of the environment black bodies.
