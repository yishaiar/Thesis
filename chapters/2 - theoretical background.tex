...
Heat flow causes thermal energy coupling from the environment. It can be shown that for an ideal gas with specific heat capacity $c_v$, the heat flow propagation distance is $\Delta x ^2  = \frac{1}{3} l^2 $, and the thermal conductivity $\lambda$ is given by:
\begin{equation}
\lambda  = \frac{l}{3}\rho c_v v    \label{eqn:heat conduction coefficient}
\end{equation}
The thermal power coupled to cross section $A$ is given by Fourier's law of heat conduction:
\begin{equation}
...
\end{equation}
Where $T$ is the environment temperature and $T_0$ is the black body temperature. Eq.~\ref{eqn: Stefan–Boltzmann power} shows that the net absorbed power is depended on the temperature difference from the environment.
