...
\end{equation}
Due to Brownian motion particles with mass $m_0$ collide into cross section $A$ of mass $m_1$, at an average collision rate $R$. Assuming all collisions are elastic and are at the same direction (the worst case), each particle cause momentum exchange of $2 m_0 v$, resulting with force due to momentum exchange of $F = 2 R m_0 v$. Due to the collisions the velosity of $m_1$ is $u = \frac{2Rm_0}{m_1}$. Thus, causing Brownian coupling power $p$, which is given by:
\begin{equation}
