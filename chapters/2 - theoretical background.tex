...
\end{equation}
\par\noindent
Where $P_{ac}$ is the actual pressure, $P$ is the average pressure, $k$ is the acoustic wave number and $\omega$ is the wave angular frequency. The acoustic intensity at room pressure is given by:
\begin{equation}
I_0 = P_{ac}^2 \propto P_0^2      \label{eqn:acoustic intensity}
\end{equation}
Acoustic energy is carried by the acoustic waves, the acoustic power $p$ is the rate acoustic energy is emitted, given by:
\begin{equation}
p = AI = AP_{ac} u  = A P_{ac}\sqrt{\frac{K_s}{\rho}}     \label{eqn:acoustic_intensity}
\end{equation}
