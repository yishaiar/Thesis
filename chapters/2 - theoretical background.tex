...
\end{equation}
Due to Brownian motion, particles with mass $m_0$ collide at an average collision rate $R$, into cross section $A$ of mass $m_1$. Assuming all collisions are elastic and are at the same direction (they do not counter act, the worst case), each particle cause momentum exchange of $2 m_0 v$, resulting with force on $m_1$ due to momentum exchange of $F = 2 R m_0 v$. Due to the collisions the velosity of $m_1$ is $u = \frac{2Rm_0}{m_1}$. Thus, causing Brownian power $p$, which is given by:
\begin{equation}
p = F\cdot u =  2 R m_0 v \cdot \frac{2Rm_0}{m_1} = 4 R^2 m_0^2  \frac{v}{m_1} = 4 (\frac{N}{V} v A)^2 (\frac{M}{N_A})^2  \frac{v}{m_1}=4 A^2 P^2\frac{1}{m_1}  (\frac{M}{k_B T N_A})^2  \frac{(\frac{3 N_A k_B T}{M})^{3/2}}{m_1}
