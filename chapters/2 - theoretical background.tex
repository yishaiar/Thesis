...
Radiative cooling is a phenomenon in which a black body loses heat by thermal radiation to remain in thermal equilibrium at constant temperature $T$. The thermal radiation energy over all frequencies per surface area $J$ is given by the Stefan–Boltzmann law:
\begin{equation}
J = \sigma\epsilon T^4  \label{eqn: Stefan–Boltzmann law}
\end{equation}
Where $\sigma$ is the Stefan–Boltzmann constant, and the emissivity $\epsilon$ is a value between $0-1$ which defines how the black body is close to being an ideal black body.
\par\noindent 
The difference between absorbed and emitted power is the net absorbed power $p$, given by: 
\begin{equation}
