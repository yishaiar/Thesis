...
\end{equation}
Where $P$ is the gas pressure, $V$ is the medium's volume, $R$ is the ideal gas constant, $T$ is the gas temperature and $k_B$ is the Boltzmann constant. The number of particles $N$ is a product of the number of moles $n$ and the Avogadro constant $N_A$. For an ideal gas the number density $\rho$ is given by:
\begin{equation}
...
\end{equation}
Due to Brownian motion particles collide into cross section $A$, at an average collision rate $R$, each particle cause momentum exchange of $2 m_0 v$, resulting with a constant force $F$. Thus, causing Brownian coupling power $p$, which is given by:
\begin{equation}
p = F\cdot v = R\cdot 2 m_0 v \cdot v = \frac{n}{V} v A \cdot 2 \frac{M}{N_A} v \cdot v =  \frac{6 A P}{ N_A} (\frac{3RT}{M})^{1/2}
