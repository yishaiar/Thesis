...
\end{equation}
Where $\sigma$ is the Stefan–Boltzmann constant, and the emissivity $\epsilon$ defines how close the black body to an ideal black body. The differences from the ideal black body is due to the energy levels and vibration modes of the material surface, which are temperature and wavelength dependent. Emissivity is typically proportional to the temperature with material specific wavelength dependency. 
\par\noindent
It can be shown that the mean energy emitted photon by a black body is given by:
The net absorbed power by the black body $p$ is given by: 
\begin{equation}
p =A\cdot(j_{absorb}-j_{emit})= A \cdot[\epsilon_1\sigma T^4-(\epsilon\sigma T^4+(1-\epsilon)\epsilon_1\sigma T^4)] = A \cdot[\epsilon_1\sigma T^4-\epsilon\sigma T^4+\epsilon\epsilon_1\sigma T^4-\epsilon_1\sigma T^4] = \epsilon (T_0,\lambda)\sigma [T^4-T_0^4]  \label{eqn: Stefan–Boltzmann power}
\end{equation}
