...
\FloatBarrier
\par\noindent
The net gravitational torque for a balanced torsion pendulum is constisted of two inverse torques that nearly counterbalance each other.
\begin{equation}
...
\begin{equation}
h(l)-h(-l)\approx h'(0)\cdot 2l,\quad h'(0) = \frac{3x}{r_0^5}\label{eqn:aproximation}
\end{equation}
\begin{equation}
...
frequency, angle is propotional to preiod length $T$, and pendulum length $l$ drops out, indicating that small sensors work as well as large ones as long as their periods are the same.
\par\noindent
