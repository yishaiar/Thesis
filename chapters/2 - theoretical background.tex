...
Black body radiation is a phenomenon in which a black body loses heat by thermal radiation to remain in thermal equilibrium at constant temperature $T_0$. The radiation power $p_{emit}$ over all wavelengths is given by the Stefan–Boltzmann law:
\begin{equation}
p_{emit} = A\epsilon\sigma T_0^4  \label{eqn: Stefan–Boltzmann law}
\end{equation}
...
\begin{equation}
p =p_{absorb}-p_{emit}= A \epsilon (T,\lambda)\sigma [T^4-T_0^4]  \label{eqn: Stefan–Boltzmann power}
\end{equation}
