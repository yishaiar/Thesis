...
\end{figure}
\par\noindent
The mass can be assumed to be a profile estimated as a long thin rod. When displaced from the equilibrium, there are two sources of torque; the displacing torque and a liniar restoring torque at the opposite direction. At equilibrium, when balance stabilizes in a specific angle, the two sources of torque cancel each other out. With small angles the pendulum obeys Hooke’s law, when $r$ is the rod length.
\begin{equation}
\tau = -\kappa\theta = rF    \label{eqn:Hooke_law}
\end{equation}
The moment of innertia of a rod with lemgth $L$ oscillating around a pivot is:
\begin{equation}
