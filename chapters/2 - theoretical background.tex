...
\par\noindent
Due to Brownian motion, gas particles with mass $m_0$ and velosity $v$ collide at an average collision rate of $R= \frac{N}{V} v A$, into cross section $A$ of mass $m_1$. 
par\noindent
Assuming all collisions are elastic and are at the same direction (the worst case they do not counter act each other), each particle exchange momentum of $dp = 2 m_0 v$ with $m_1$. Due to the collision the velosity of $m_1$ is $u = \frac{2m_0 v}{m_1}$. Thus, causing Brownian power $p$, which is given by:
\begin{equation}
p = F\cdot u =  Rdp\cdot u R\Delta t=dp 2 m_0 v \cdot \frac{2Rm_0v}{m_1} = 4 R^2 m_0^2 v^2  \frac{1}{m_1} = 4 (\frac{N}{V} v A)^2 (\frac{M}{N_A})^2 v^2  \frac{1}{m_1}= \frac{36}{m_1} (A k_B T \rho_N)^2 
