...
\end{equation}
Where $\sigma$ is the Stefan–Boltzmann constant, and the emissivity $\epsilon$ defines how close the black body to an ideal black body ($\epsilon =1$). The differences from the ideal black body is due to the energy levels and vibration modes of the material surface, which are temperature and wavelength dependent. Kirchhoff's law states that the emissivity equals the absorption of the material.
\par\noindent
The black body absorbs radiation from black bodies around. For a black body with temperature $T_0$ absorbing radiation of an ideal black body with temperature $T$, the net absorbed power $p$ of is given by: 
\begin{equation}
p = A\cdot(j_{absorb}-j_{emit}-j_{reflect})= A \cdot[\sigma T^4-\epsilon\sigma T_0^4-(1-\epsilon)\sigma T^4)] = A \sigma\epsilon[ T^4- T_0^4]   
