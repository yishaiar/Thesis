...
\begin{equation}
\tau = lF cos\theta -\kappa\theta    \label{eqn:Hooke_law}
\end{equation} 
...
\noindent
The string torsion coefficient $\kappa $ could be estimated, assuming homogeneity of a circular string. Where $G$ is the material shear modulus, $J$ is the circular second moment of area ($Jzz$) and $h$ is the string length.
\begin{equation}
...
\end{equation}
The field caused by a mass $M$ at a specific point is calculated by measuring the gravitational force. The gravitational force is caused by a known mass $M$ (a test mass). Test mass $M$ is much larger than each mass $m$, which ensures that there is a negligible influence on the behaviour of $M$.  
...
\FloatBarrier
\par\noindent
The net gravitational torque for a balanced torsion pendulum is consisted of two inverse torques that nearly counterbalance each other.
\begin{equation}
\tau_g = lF_g(r_1)cos\theta_1 - lF_g(r_2)cos\theta_2 = ldGmM(\frac{1}{r_1^3} - \frac{1}{r_2^3})     \label{eqn:gravitation_torque}
\end{equation}
\begin{equation}
h(l) = \frac{1}{(d^2 +(x-l)^2)^{3/2}};\quad h(l) = \frac{1}{r_1^3},\quad	h(-l) = \frac{1}{r_2^3},\quad	h(0) = \frac{1}{r_0^3}    \label{eqn:gravitation_torque}
\end{equation}
\par\noindent
Since $l<<d,x,r_0$ one can approximate:
\begin{equation}
h(l)-h(-l)\approx h'(0)\cdot 2l,\quad h'(0) = \frac{3x}{r_0^5}\label{eqn:approximation}
\end{equation}
\begin{equation}
\tau_g = ldGmM[h(l)-h(-l)]\approx \frac{6l^2GmMdx} {r_0^5} = \frac{6l^2GmMcos\theta sin\theta}{r_0^3}      \label{eqn:gravitation_torque}
\end{equation}

\par\noindent
Assuming no friction or other damping force (a simple harmonic oscillator) when a mass is introduced, there are two sources of torque in the system; torque by the mass gravitational force, and restoring torque caused by the wire torsion. 
\par\noindent
At equilibrium, when balance has been stabilized at an angle $\theta$, these two torques are cancelled out.
\begin{equation}
\kappa\theta = \tau_g(r_0)\approx \frac{6l^2GmMcos\theta sin\theta}{r_0^3}    \label{eqn:gravitation_torque}
\end{equation}
\begin{equation}
\overline{\theta} = \frac{\tau}{\kappa} = \frac{\tau}{I\omega_0^2}    \label{eqn:theta average}
\end{equation}
\begin{equation}
\overline{\theta} \approx \frac{3GMcos\theta sin\theta}{\omega_0^2 r_0^3}    \label{eqn:theta average}
\end{equation}
\par\noindent
...
At low pressures, there are more gas molecules adsorbed on the chamber surface than floating in the chamber. When pressure is below vapour pressure there's a desorption of gas molecules (primarily water) from the material.
\par\noindent
...
\end{figure}
\begin{equation}
u(t) = K_Pe(t)+K_I\int_{0}^{t}e(t)+K_D\frac{de(t)}{dt}   \label{eqn:PID_eqn}
\end{equation}
