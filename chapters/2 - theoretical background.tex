...
\end{equation}
Where $P$ is the gas pressure, $V$ is the medium's volume, $R$ is the ideal gas constant, $T$ is the temperature and $k_B$ is the Boltzmann constant. The number of particles $N$ is a product of the number of moles $n$ and the Avogadro constant $N_A$. For an ideal gas composed of particles with a molar mass $M$ the density $\rho$ is given by:
\begin{equation}
\rho(P) = \frac{MP}{RT}     \label{eqn:ideal density}
\end{equation}
For a diluted gas the viscosity $\mu$ is given by:
\begin{equation}
\mu(\rho) = \alpha l \rho \sqrt{\frac{2\pi k_B  T}{\pi m}}      \label{eqn:viscosity}
\end{equation}
Where $\alpha$ is a numerical constant, $l$ is the mean free path of the gas particles and $m$ is the molecular mass. The viscosity is density dependent, resulting with an ideal gas pressure dependency for viscosity $\mu(P)$
...
\end{equation}
Where $C_d$ is the drag coefficient, $A$ is the cross section of the area, and $b$ is the drag force proportionality factor. The Reynolds number $Re$ which is used to characterize types of gas flow is given by:
\begin{equation}
Re= \frac{\rho v d}{\mu}     \label{eqn:Reynolds number}
\end{equation}
Thus, for a constant temperature $T$ the pressure $P$ is proportional to the gas density $\rho$ and to the Reynolds number $Re$. 
When pressure decreases and the Reynolds number approaches zero it can be shown that $C_d \rightarrow \frac{24}{Re}$, resulting with Stokes' drag:
\begin{equation}
...
There is thermal energy coupling from the environment on the sides of the medium, phonons are penetrating and pass energy to the gas particles inside the medium. It can be shown that for an ideal gas with a mean free path $l$ the heat flow propagating distance is $\Delta x ^2  = \frac{1}{3} l^2 $, and the thermal conductivity is:
\begin{equation}
\lambda  = \frac{l}{3}\rho c_v v = \frac{l}{3} \frac{MP}{RT} c_v v   \label{eqn:heat conduction coefficient}
\end{equation}
The rate of heat flow $\dot{Q}$ (Fourier's law of heat conduction) for an ideal gas is:
\begin{equation}
\dot{Q} = \lambda A \frac{\Delta T}{\Delta x}  = \frac{l}{3} \frac{MP}{RT} c_v v A \frac{\Delta T}{\sqrt{\frac{1}{3} l^2 }} = \frac{M P c_v v A \Delta T}{\sqrt{3}RT}   
\label{eqn:heat conduction}
\end{equation}
\par\noindent
The heat flow is propagating a distance of $\Delta x$ through a material with surface area $A$, thermal conductivity $\lambda$ and a specific heat capacity $c_v$ .The heat flow rate $\dot{Q}$ is proportional to the temperature difference $\Delta T$. The heat flowing in to the medium is the thermal energy coupled from the environment, eq.~\ref{eqn:heat conduction} shows that the rate of thermal energy coupling is proportional to the gas pressure.
