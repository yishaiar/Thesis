...
\end{equation}
Where $\sigma$ is the Stefan–Boltzmann constant, and the emissivity $\epsilon$ defines how close the black body to an ideal black body. The differences from the ideal black body is due to the energy levels and vibration modes of the material surface, which are temperature and wavelength dependent. Emissivity is typically proportional to the temperature with material specific wavelength dependency. The net absorbed power by the black body $p$ is given by: 
\begin{equation}
p =A\cdot(j_{absorb}-j_{emit})= A \epsilon (T_0,\lambda)\sigma [T^4-T_0^4]  \label{eqn: Stefan–Boltzmann power}
\end{equation}
Where $T$ is the environment temperature and $T_0$ is the black body temperature. Eq.~\ref{eqn: Stefan–Boltzmann power} shows that the net absorbed power is proportional to the to the temperature difference from the environment.
