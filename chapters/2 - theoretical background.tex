...
\par\noindent
Solving eq.~\ref{eqn:damped_motion_equation_2}, with different $\xi$ values the oscillator motion could be either undamped, overdamped, critically damped or underdamped. The oscillator motion equations when solved for the boundaries $\theta(0) = \theta_{max}$ and $\dot{\theta}(0) = 0$:
\par\noindent
