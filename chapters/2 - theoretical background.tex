...
\end{equation}
The random collisions cause momentum exchange, resulting with force which is a product of the average particle momentum exchange $2mv$ and the RMS of the rate $\sqrt{R}$: 
\begin{equation}
...
\begin{equation}
p_{emit} = \sigma\epsilon(T) A T_0^4  \label{eqn: Stefan–Boltzmann law}
\end{equation}
Where $\sigma$ is the Stefan–Boltzmann constant, and the emissivity $\epsilon(T)$ is a value which defines how close the black body to an ideal black body, which is typically proportional to the temperature. The differences from ideal black body is due to the atoms energy levels and vibration modes, which are temperature dependent. The net absorbed power $p$ is given by: 
\begin{equation}
p = p_{absorb}-p_{emit} = \sigma\epsilon(T) A (T^4-T_0^4)  \label{eqn: Stefan–Boltzmann power}
\end{equation}
