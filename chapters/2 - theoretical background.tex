...
\begin{equation}
p_{emit} = A\sigma\epsilon T_0^4  \label{eqn: Stefan–Boltzmann law}
\end{equation}
Where $\sigma$ is the Stefan–Boltzmann constant, and the emissivity $\epsilon$ is a value which defines how close the black body to an ideal black body. The differences from the ideal black body is due to the energy levels and vibration modes of the surface, which are temperature and wavelength dependent resulting with. Emissivity is typically proportional to the temperature with material specific wavelength dependency. The net absorbed power by the balck body $p$ is given by: 
\begin{equation}
p =p_{absorb}-p_{emit}= A \sigma\epsilon(T,\lambda) [T^4-T_0^4]  \label{eqn: Stefan–Boltzmann power}
\end{equation}
...
\begin{equation}
I( t,\vec{r}) = P_{ac}\cdot v = P\cdot \psi( t,\vec{r}) \cdot v( t,\vec{r})      \label{eqn:acoustic_pressure}
\end{equation}
\par\noindent
Where $P_{ac}$ is the instantaneous pressure, given by the effect of the propagating acoustic wave $\psi( t,\vec{r})$ on the average pressure $P$. The energy carried by the acoustic wave is also effected by the gas particles velocity $v$. When integrating over the wave time period $T$, the average acoustic power $p$ is given by:
\begin{equation}
p(\vec{r}) =  A <I>=\frac{AP}{T}\int_0^T \psi( t,\vec{r}) \cdot v( t,\vec{r})dt  
\label{eqn:acoustic power}
\end{equation}
When the gas pressure is reduced, the acoustic pressure is lower and less power is carried.
