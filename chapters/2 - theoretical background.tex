...
\end{equation}
\par\noindent
Where $P_{ac}$ is the instantaneous pressure, given by the effect of the propagating acoustic wave $\psi( t,\hat{r})$ on the average pressure $P$ and $v$ is the gas particles velocity. The average acoustic power $p$ is given by:
\begin{equation}
p(\hat{r}) =  A\cdot <I>=\frac{AP}{T}\int_0^T \psi( t,\vec{r}) \cdot v( t,\vec{r})dt  
\label{eqn:acoustic power}
\end{equation}
Where $v$ is the gas particles velocity. When the gas pressure is reduced, the acoustic pressure is lower and less power is carried.
