...
\par\noindent
The apparatus, as seen below, is constructed by a torsional pendulum and a nearby mass $M$, creating a net torque on the pendulum. The torsional pendulum is formed of two masses $m$ connected by a rigid, massless thin rod, with length of $2l$.

\par\noindent
The net gravitational torque $\tau_g$ for a balanced torsion pendulum is the sum of two inverse, nearly counterbalancing gravitational torques, from each mass $m$:
\begin{equation}
...
\par\noindent
The net torque is proportional to the difference of the functions $h(l)-h(-l)$, where:
\begin{equation}
h(l) = \frac{1}{(d^2 +(x-l)^2)^{3/2}} \label{eqn:gravitation_torque}
\end{equation}

%\begin{equation}
%h(l) = \frac{1}{r_1^3},\quad	h(-l) = \frac{1}{r_2^3},\quad	h(0) = \frac{1}{r_0^3}   \label{eqn:gravitation_torque}
%\end{equation}
...
\par\noindent
Assuming no friction or other damping force (a simple harmonic oscillator) when a mass $M$ is introduced, there are two sources of torque in the system; the net torque caused by the gravitational force $\tau_g$, and an inverse restoring torque caused by the wire torsion.
\begin{equation}
\tau = \tau_g - \kappa\theta     \label{eqn:gravitation_torque}
\end{equation} 
\par\noindent
At equilibrium, when balance has been stabilized, these two torques are cancelled out:
\begin{equation}
\tau = 0 \rightarrow\kappa\theta = \tau_g    \label{eqn:gravitation_torque}
\end{equation}
\begin{equation}
