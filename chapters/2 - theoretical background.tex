...
\par\noindent
A torsional pendulum, shown in fig.~\ref{fig:torsion_pendulum}, is an oscillator made of a mass hung by a string from a fixed point allowing it to swing freely. When the mass is displaced from the equilibrium angle, the pendulum has a linear restoring torque, rotating the mass back to the equilibrium position.
\begin{figure}[htbp]
	\centering
...
\end{figure}
\FloatBarrier
The restoring tourqe is given by $\tau = -\kappa\cdot\theta$. The twisted string is causing the tourqe $\tau$, therefore the tourqe is proportional to the string torsion coefficient $\kappa$ and the angle $\theta$. Assuming homogeneity of a circular string, the string torsion coefficient $\kappa$ can be estimated by:
\begin{equation}
...
\end{equation}
The damping time $\tau$ is the time it takes to damp to $\frac{1}{e} \approx 36.8\%$ of the initial amplitude. The damping time is given by:
\begin{equation}
\tau = \frac{1}{\xi\omega_0} = \frac{1}{\frac{b}{2\sqrt{I\kappa}}\sqrt{\frac{\kappa}{I}} }= \frac{2I}{b}  \label{eqn:damping_time}
\end{equation}

\par\noindent
Solving eq.~\ref{eqn:damped_motion_equation_2}, the oscillator motion could be either undamped, overdamped, critically damped or underdamped, the motion is determined by the damping ratio value. 
...
\end{equation}
When the the oscillator is highly underdamped ($\xi << 1$) the damping time is very large and frequency change negligible, resulting with an approximately undamped oscillator.
\par\noindent
...
\begin{figure}[htbp]
	\centering
	\fbox{\includegraphics[scale=0.2]{\main/images/2 - theoretical background/damp.png}}
	\caption[Damped oscillators comparison]{Damping comparison for oscillator with T = 86 [s]}
...
\begin{figure}[htbp]
	...
	\caption[Cavendish apparatus]{Cavendish apparatus see \cite{howell2019}}
	\label{fig:Cavendish apparatus}
\end{figure}
\FloatBarrier
\par\noindent
The Cavendish apparatus, shown in fig.~\ref{fig:Cavendish apparatus}, measures the gravitational force of test mass $M$ on masses $m$. The apparatus is constructed by a torsional pendulum and a nearby test mass $M$, which creates a net torque on the pendulum. 
\par\noindent
...
\end{equation}
Where $h(l) = \frac{1}{r_1^3},\quad	h(-l) = \frac{1}{r_2^3},\quad	h(0) = \frac{1}{r_0^3}$, and the derivative is: $h'(0) = \frac{3x}{r_0^5}$. If $l<<d,x,r_0$ one can approximate:
\begin{equation}
...
Assuming no friction or other damping force (simple harmonic motion) when a test mass $M$ is introduced, there are two sources of torque in the system; the net torque caused by the gravitational force $\tau_g$ (eq.~\ref{eqn:net_gravitation_torque_approx}), and a restoring torque in the opposite direction caused by the wire torsion (eq.~\ref{eqn:undamped_motion_equation}):
\begin{equation}
\tau = \tau_g - \kappa\theta = \tau_g - (I\omega_0^2)\theta    \label{eqn:gravitation_torque}
\end{equation}
\par\noindent
At equilibrium, the two sources of torque cancel each other out $\tau_g =  \kappa\theta$. Accordingly, with moment of inertia $I$ (eq.~\ref{eqn:moment_inertia}) the average tilt angle $\overline{\theta}$ is given by:
\begin{equation}
...
\end{equation}
Where $P$ is the gas pressure, $V$ is the medium's volume, $R$ is the ideal gas constant, $T$ is the gas temperature and $k_B$ is the Boltzmann constant. The number of particles $N$ is a product of the number of moles $n$ and the Avogadro constant $N_A$. For an ideal gas the mass density $\rho$ is given by:
\begin{equation}
...
\label{eqn:total Brownian energy}
\end{equation}
\par\noindent
Eq.~\ref{eqn:total Brownian energy} shows that the Brownian motion energy is proportional to the gas pressure. 
...
\begin{equation}
I( t,\vec{r}) = P_{ac}( t,\vec{r})\cdot u( t,\vec{r}) = P \psi( t,\vec{r}) \cdot P\phi( t,\vec{r})      \label{eqn:acoustic_pressure}
\end{equation}
\par\noindent
Where $P_{ac}$ is the instantaneous acoustic pressure, given by the effect of the propagating acoustic wave $\psi( t,\vec{r})$ on the average pressure $P$. The energy carried by the acoustic wave is also effected by the gas particles velocity $u$ which is pressure dependent. When integrating over the wave time period $T$, the average acoustic power $p$ is given by:
\begin{equation}
p(P,\vec{r}) =  A <I>=\frac{AP^2}{T}\int_0^T \psi( t,\vec{r}) \cdot \phi( t,\vec{r})dt  
\label{eqn:acoustic power}
\end{equation}
The typical reference intensity value for air at room pressure $P_0  \approx 1\cdot10^{5}[pa] $ and room temperature is $<I_0> = 1\cdot10^{-12}[\frac{W}{m^2}]$, resulting with power given by:
\begin{equation}
p \approx  A\frac{<I_0>}{P_0^2}\cdot P^2 \approx 1\cdot10^{-22}AP^2
\label{eqn:acoustic power1}
\end{equation}
Eq.~\ref{eqn:acoustic power} shows that the acoustic power is pressure dependent. When the gas pressure is reduced, the acoustic pressure is lower and less power is carried.
...
\begin{equation}
j_{emit}= \epsilon\sigma T_0^4  \label{eqn: Stefan–Boltzmann law}
\end{equation}
Where $\sigma$ is the Stefan–Boltzmann constant, and the emissivity $\epsilon$ defines how close the black body to an ideal black body ($\epsilon =1$). The differences from the ideal black body is due to the energy levels and vibration modes of the material surface, which are temperature and wavelength dependent. 
\par\noindent
While the black body emits $j_{emit}$, black bodies around also radiate $j$ with their temperature $T$. Some of the incident flux is reflected $j_{reflect}$, with reflectance given by Kirchhoff's law $r=1-\epsilon$. For black body with incident radiation of an ideal black body, the net absorbed power $p$ is: 
\begin{equation}
p = A\cdot[j-j_{reflect}-j_{emit}]= A \cdot[\sigma T^4-(1-\epsilon)\sigma T^4-\epsilon\sigma T_0^4)] = A \sigma\epsilon[ T^4- T_0^4]   
\label{eqn: Stefan–Boltzmann power}
\end{equation}
Eq.~\ref{eqn: Stefan–Boltzmann power} shows that the net absorbed power is depended on the temperature difference from the environment. 
\par\noindent
Additionally, the incident radiation of the environment $A\sigma T^4$ causes momentum and energy exchange due to photon collisions, similar to the gas particles Brownian motion. The collisions are depended on the environment temperature.
...
\begin{figure}[htbp]
	\centering
	\fbox{\includegraphics[scale=0.25]{\main/images/2 - theoretical background/pid_diagram_powerpoint.jpg}}
