...
\end{equation}
\par\noindent
Where $P_{ac}$ is the local actual pressure, $P$ is the average pressure, $k$ is the acoustic wave number and $\omega$ is the wave angular frequency. The acoustic intensity is $I = P_{ac}^2 \propto P^2$. The acoustic power $p$ is the rate acoustic energy is emitted, given by:
\begin{equation}
p = A\cdot I \propto P^2
\label{eqn:acoustic power}
\end{equation}
When the gas pressure is reduced, the acoustic pressure is lower and less power is carried.
 at room pressure $P_0\approx 1\cdot 10^{5} [Pa]$ the acoustic intensity is typically $I_0\approx 1\cdot 10^{-12}[\frac{W}{m^2}]$. 
 
\begin{equation}
A\cdot I_0\frac{P^2} {P_0^2} \approx A\cdot 1\cdot 10^{-12}\frac{P^2} {1\cdot 10^{5\cdot 2}}\approx AP^2\cdot 10^{-22}
\label{eqn:acoustic power}
\end{equation}
