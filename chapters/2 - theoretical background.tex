...
The pendulum can be assumed to be of two masses $m$ connected by a rigid, massless thin rod, with length of $2l$, and a pivot at distance $l$ from each side. When displaced from the equilibrium, there are two sources of torque; the displacing torque caused by an external force, and a liniar restoring torque at the opposite direction.
\begin{equation}
...
\begin{equation}
\tau = \kappa\theta = lF = l\frac{GmM}{r^2}    \label{eqn:gravitation_torque}
\end{equation}
\begin{equation}
\overline{\theta} = \frac{\tau}{\kappa} = \frac{\tau}{I\omega_0^2}    \label{eqn:theta average}
\end{equation}
The torsion string coefficient $\kappa$ could be estimated from the oscillations period.
