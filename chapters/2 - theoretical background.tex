...
\par
The radiation pressure force depends on the angle of surface compare to electromagnetic field, surface material, and the power of the light source.
Light radiant flux (measured in watts) denoted $\Theta$, surfce intesity reflectance parameter denoted R, A is the surface area, $\eta$ is the coupling effitiancy, c is the spped of light.
\begin{equation}
P_{incident} = \frac{\frac{\Theta}{A}}{c} = \frac{\eta\cdot \Theta_e^s}{{A\cdot c}} \label{eqn:energy-mass-equivalence-relation}
\end{equation}

Assuming the light Direction is is perpendicular to the surface, and the light variation over the surface is negligible.

\begin{equation}
F = P_{total}\cdot A = (P_{incident}+P_{emitted})\cdot A = P_{incident}\cdot A\cdot(1+R)\label{eqn:energy-mass-equivalence-relation}
\end{equation}
\begin{equation}
F = \frac{\eta\cdot\Theta_{source}}{{A\cdot c}}\cdot A\cdot(1+R) = \frac{\eta\cdot\Theta_{source}\cdot(1+R)}{{c}} \label{eqn:energy-mass-equivalence-relation}
\end{equation}
\begin{equation}
