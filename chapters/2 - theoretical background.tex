...
\par\noindent
An optical signal with $N$ photons of frequency $\nu$ is with power $p_{signal} = N h \nu$, where $h$ is the Planck constant. Since the shot noise is a Poisson process, the shot noise is the power of the root mean square (RMS) of the number of events (number of photons) $p_{shot} = \sqrt{N} h \nu$.
\par\noindent
...
\end{equation}
Where $\beta$ is a numerical constant and $l$ is the mean free path of the gas particles. The molecular velocity $\widetilde{v}$ of gas particles with mass $m_0$ is given by the Maxwell-Boltzmann distribution, it can be shown that the average kinetic energy of a gas particle $<E_k>$ is given by:
\begin{equation}
<E_k>=<\frac{m\widetilde{v}^2}{2}> = \int_{0}^{\infty}\frac{m\widetilde{v}^2}{2}f(\widetilde{v})d\widetilde{v} =  \frac{3k_BT}{2}    \label{eqn:average kinetic}
\end{equation}
\par\noindent
...
\end{equation}
Due to Brownian motion particles collide into cross section $A$, at an average collision rate $R$, each particle cause momentum exchange of $2mv$, resulting with a constant force $F$. Thus, causing Brownian coupling power $p$, which is given by:
\begin{equation}
p = F\cdot v = R\cdot 2mv \cdot v = \frac{N}{V} v A \cdot 2mv\cdot v = 2m\frac{P}{K_B T}  A   (\frac{3RT}{M})^{3/2} = 6 A P    \sqrt{\frac{3RT}{M}}     \label{eqn:Brownian power}
\end{equation}
...
\begin{equation}
p = p_{absorb}-p_{emit} =A\cdot(j_{absorb}-j_{emit})= A \sigma\epsilon(T_0) (T^4-T_0^4)  \label{eqn: Stefan–Boltzmann power}
\end{equation}
