...
\begin{frame}{Quality factor}
	...
	\begin{itemize}	
		...
		\item when $Q<0$; coupled noise larger than damping, causing a driven oscillator.
		Coupled noise estimation from the measured damped amplitude: $p \rightarrow \frac{ \theta_{max}^{PID}\pi\tau_{max}}{T}$, resulting with $Q(\theta_{max}^{PID},p)	\rightarrow 0$. 				
	\end{itemize}
\end{frame}
