...
\begin{frame}{Previous laser setup}
	...
	\begin{itemize}		
		...
		\item Modulation range of $10000$ steps with frequency of $1 [Hz]$.
...
\begin{frame}{\hypertarget{frame:LED PWM circuit}{LED PWM circuit}}
	...
	\begin{itemize}		
		...
		\item Modulation range of 8-bit duty-cycle ($D(t)$).
...
\begin{frame}{Resolution limit}
	...
	\begin{itemize}	 
		...
		\item Might allow to damp down to $\theta_{max}^{PID}\approx 2\cdot 10^{-14}$.
...
\begin{frame}{\hypertarget{frame:Quality factor 1}{Quality factor}}
	...
	\end{center}
	\begin{itemize}			
		\item Ratio of  energy stored to energy dissipated per cycle.
		\item Assuming negligible friction, dissipation is the work.
		\item The difference of PID work and environment noise power. 
		\item Ratio of the energy stored to dissipated per cycle.
		\item Noise and amplitude depended: $Q =  \frac{2\pi\cdot(\frac{\kappa\cdot\theta(t)^2}{2}+\frac{I\cdot\dot{\theta}(t)^2}{2})}{\int_0^T[\tau_{PID}(t)\cdot\dot{\theta}(t) - p]dt} $.
