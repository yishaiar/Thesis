...
\begin{frame}{Light emitting diode (LED)}
	...
	\begin{itemize}		
		...
		\item Illumination angle of $45^0-120^0$ and hard to focus.
...
\begin{frame}{LED circuit}
	...
	\begin{itemize}		
		...
		\item $\tau_{PID}(t)  \approx   3\cdot 10^{-13}(D_1(t) -D_2(t)) $
...
\begin{frame}{LED circuit}
	...
	\end{center}
	\begin{itemize}		
		\item Damping is amplitude depended; accelerates as amplitude gets smaller.
		\item Damping time of $3700[s]$ for $ 60 [\mu rad] \rightarrow 10[\mu rad] $, compared to $250[s]$ to reach $ 0.2[\mu rad]$.
		\item PID is able to damp up to an amplitude of $\theta_{max}^{PID}= 0.05[\mu rad]$ with an (RMS) amplitude kept by the PID over time of $\overline{\theta_{max}^{PID}} = 0.2 [\mu rad]$
